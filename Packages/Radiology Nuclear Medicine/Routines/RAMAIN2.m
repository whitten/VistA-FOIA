...
2 ;;Procedure Enter/Edit
 ...
 ; RA PROCEDURE option
 N RAENALL,RAY,RAFILE,RAINA,RASTAT,RAXIT S (RAENALL,RAXIT)=0
 N RADIO,RAPTY,RAASK,RAROUTE,REM ;used by the edit template
 F  D  Q:$G(RAXIT)
 ...
 .. Q
 . S RAINA=$P($G(^RAMIS(71,RADA,"I")),"^")
 . S RASTAT=$S(RAINA="":1,RAINA>DT:1,1:0)_"^"
 ...
 . S RAINA=$P($G(^RAMIS(71,RADA,"I")),"^")
 . S RASTAT=RASTAT_$S(RAINA="":1,RAINA>DT:1,1:0)
 ...
 .. Q
 . I RAINA="",($P(RAPROC(0),"^",6)="P"),('$O(^RAMIS(71,RADA,4,0))) D
 ...
 .. D ^DIE K D,D0,D1,DA,DI,DIC,DIE,DQ,DR
 .. Q
 . I $P($G(^RA(79.2,+$P(RAPROC(0),"^",12),0)),"^",5)="Y",(+$O(^RAMIS(71,RADA,"NUC",0))) D VRDIO(RADA)
 . I "^B^P^"[(U_$P(RAPROC(0),"^",6)_U),($P(RAPROC(0),U,9)]"") D
 .. K %,D,D0,DA,DE,DIC,DIE,DQ,DR
 .. S DA=RADA,DIE="^RAMIS(71,",DR="9///@" D ^DIE
 .. W !!?5,"...CPT code deleted because "_$S($P(RAPROC(0),"^",6)="B":"Broad",1:"Parent")_" procedures",!?5,"should not have CPT codes.",!,$C(7)
 .. Q
 . K %,%X,%Y,C,D,D0,D1,DA,DE,DI,DIE,DQ,DR,RAIMAG,RAMIS,RAPROC,X,Y
 . ;If edited, or the entry is new & active send Orderable Item Update
 . ;message to CPRS if the Order Dialog file exists.
 . I ('+$P(RAY,"^",3)!(+$P(RAY,"^",3)&(+$P(RASTAT,"^",2)))),($$ORQUIK^RAORDU()=1) D
