RASTED ;HISC/CAH,FPT,GJC,SS AISC/TMP,TAC,RMO-Edits for status tracking ;6/2/98  15:43
 ;;5.0;Radiology/Nuclear Medicine;**1,10,18,28**;Mar 16, 1998
...
START F I3=1:1:11 S @$P("RADFN^RADTI^RACNI^RANME^RASSN^RADATE^RADTE^RACN^RAPRC^RARPT^RAST","^",I3)=$P(Y,"^",I3)
 I '$D(^RA(72,+RAST,0)) W *7,"Invalid status for case #: ",RACN R X:3 Q
 S RAST0=^RA(72,+RAST,0) I $P(RAST0,"^",3)=9 W *7,!,"Exam is already complete!!" R X:3 Q
 S X1=""
...
NEXT I '$D(RANEXT) S DIC("A")="Enter Next Status: ",DIC="^RA(72,",DIC(0)="AEFQZ",DIC("S")="I $P(^(0),U,3),$P(^(0),U,7)=$O(^RA(79.2,""B"",RAIMGTY,0))" D ^DIC K DIC Q:Y'>0  S RANEXT=Y(0),RASK=$S($D(^RA(72,+Y,.2)):^(.2),1:""),RANXT72=+Y
 I $P(RANEXT,"^")=$P(RAST0,"^") W *7,!,"Status has already been set to ",$P(RANEXT,"^") R X:3 Q
 I $$LKUP^XPDKEY(+$P(RANEXT,"^",4))]"",'$D(^XUSEC($$LKUP^XPDKEY(+$P(RANEXT,"^",4)),DUZ)) W *7,!,"You are not authorized to change to this status" R X:3 Q
 ; check if next status has order field filled in
 G:$P(RANEXT,U,3)]"" OK2
 N RANXTIEN,RALINE S RANXTIEN=$P(RAST0,U,2),$P(RALINE,"_",50)=""
 W !!?15,*7,RALINE
 W !!?15,*7,"Default Next Status (",$P(RANEXT,U),") is *NOT* active.",!?15,*7,RALINE,!
NXT S RANXTIEN=$P(^RA(72,RANXTIEN,0),U,2)
 ...
 G OK0
BAD W !?15,*7,RALINE
 W !!?18,*7,"There is no valid higher status to advance to.",!?15,*7,RALINE
KEY W !! K DIR S DIR(0)="E",DIR("A")="Press Return key to continue " D ^DIR
 K DIR,DIRUT,DUOUT Q
OK0 S RANEXT=$G(^RA(72,RANXTIEN,0)),RANXT72=RANXTIEN
OK1 W !?15,*7,RALINE,!!?18,"Next valid status is : ",$P(RANEXT,U),!?15,*7,RALINE
...
CHANGE W !!,"Name: ",RANME,?40,"Case #  : ",RACN,!,"Division : ",$S($D(^DIC(4,+$P(RADT0,"^",3),0)):$P(^(0),"^"),1:"")
 ...
 I RAPRC="Unknown" W !!?5,*7,"This record is corrupted -- the procedure is missing,",!?5,"please contact your ADPAC or IRM",! K DIR S DIR(0)="E",DIR("A")="Press RETURN to Continue" D ^DIR K DIR,DIROUT,DIRUT,DTOUT,DUOUT Q
ASK R !,"Do you wish to continue? YES// ",X1:DTIME S:X1="" X1="Y" Q:'$T!(X1["^")!("nN"[X1)
 ...
 N RAWHICH,REM,RABEFORE,RAAFTER
 ...
 Q
USER S %="A",%DUZ=DUZ W ! D ^XUVERIFY G USERQ:%=-1 I %'=1 W *7," ??" G USER
 Q
...
WHYMSG2 W !?12,*7,"The selected case belongs to a print set,",!?12,"Please use the 'Report Enter/Edit' option",!?12,"to enter data for ",$S(RA3=99:"residents/staff/diagnoses",RA3'=13:"residents/staff",1:"diagnoses"),".",!!
 Q
WHY11 Q:'+$P(RA0,"^",RA3)
