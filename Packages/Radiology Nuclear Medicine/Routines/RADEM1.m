...
EXAM D HDR S RAXIT=0
 ...
 W:$D(RAORFL) !?2," **Oral cholecystogram performed in last 7 days."
 W:$D(RACNFL) !?2,"***Exam with contrast material performed in last 3 days."
 I '$D(RACONT),('+$G(RAXIT)) R !!,"Press <RETURN> key to continue.",X:DTIME
Q K %,%H,DIC,POP,RACNFL,RAORFL,RACODE,RACONT,RABAR,RABARFL,RACHKDT,RACHKDT1,RACN,RACNI,RADATE,RADTE,RADTI,RAPR1,RAPRI,RASEQ,RAST,RASTI,RAXIT Q
 ;
...
PRT N RAESITY,RAITYPE
 ...
 I RADTE'<RACHKDT,$P(Y,"^",5)="Y" S (RABAR,RABARFL)=1,RACODE="  *"
 I RADTE'<RACHKDT,$D(^RAMIS(71,"AC",11,RAPRI)) S (RABAR,RAORFL)=1,RACODE=" **"
 I RADTE'<RACHKDT1,$P(Y,"^",10)="Y" S (RABAR,RACNFL)=1,RACODE="***"
 S RASTI=+$P(Y,"^",3)
...
HDR ; Header
 ...
 ;D HOME^%ZIS  ;IHS/ITSC/CLS 02/09/2004 don't reset
 W:$D(RAOPT("ORDEREXAM"))#2 @IOF
 W !!,"Case #",?10,"Last 5 Procedures/New Orders",?39,"Exam Date",?51,"Status of Exam",?67,"Imaging Loc."
 W !,"------",?10,"----------------------------",?39,"---------",?51,"--------------",?67,"------------"
 K RAOPT("ORDEREXAM")  ;IHS/ITSC/CLS 09/14/2004 Kill here so request prints on one page.
 Q
