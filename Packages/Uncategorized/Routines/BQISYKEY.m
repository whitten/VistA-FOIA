...
RET(DATA,USER,ROLE) ; EP - BQI GET USER ROLES
 ...
 ;
 NEW UID,II,BKEY,BKIEN,OKEY,HDR,RIEN,BQROLE
 S UID=$S($G(ZTSK):"Z"_ZTSK,1:$J)
 ...
 NEW $ESTACK,$ETRAP S $ETRAP="D ERR^BQISYKEY D UNWIND^%ZTER" ; SAC 2006 2.2.3.3.2
 ;
 S HDR="T00050BQIUSER^T00001BQIZCUSR^T00001BQIRPC^T00001BQIZCMED^T00001BQIZTXED^T00001BQIZMGR^T00001BTPWZCMGR^T00001EDIT_CANDIDATE^T00001TAX_CANDIDATE"
 S @DATA@(II)=HDR_$C(30)
 ;
 K BQROLE
 ;
...
ROL(BUSER,BQROLE) ;EP - Assemble User Information Based on input User/Role
 ...
 . ;
 . ;Check if iCare User
 . S:$G(^BQICARE(BUSER,0))'="" $P(BQUSER(BUSER),U,2)="Y"
 . ;
 ...
 .... S $P(BQUSER(BUSR),U,(POS+4))="Y"
 . ;
 . ;Check if iCare User
 . S BUSR=0 F  S BUSR=$O(^BQICARE(BUSR)) Q:'BUSR  D
 .. I $G(^VA(200,BUSR,0))="" Q
 .. S $P(BQUSER(BUSR),U,2)="Y"
 . ;
 . ;Check for BQIRPC Secondary Menu
 . D BQIRPC(BUSER,.BQROLE,.BQUSER)
 . ;
 ...
 . F POS=2:1:9 I $P(BQUSER(BUSR),U,POS)="" S $P(BQUSER(BUSR),U,POS)="N"
 . S POS=5 I $P(BQUSER(BUSR),U,POS)="Y" S $P(BQUSER(BUSR),U,(POS+4))="Y"  ; taxonomy edit candidate
 . I $P(BQUSER(BUSR),U,3)'="Y" K BQUSER(BUSR) Q
