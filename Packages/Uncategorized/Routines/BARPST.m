BARPST ; IHS/SD/LSL - PAYMENT BATCH POSTING JAN 15,1997 ; 07/14/2010
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**6,7,13,15,19,20**;OCT 26, 2005
 ;;
...
CKDATE(Z,Q,P) ;EP; NEW; CHECK COLLECTION BATCH DATE ;MRS;BAR*1.8*6 DD 4.2.4
 ...
 .;S BAR=$O(^BAREDI("I",DUZ(2),"C",$E(Z,1,30),"")) ;LIMIT TO 30 CHAR;MRS;BAR*1.8*7 IM30386  ;bar*1.8*20
 .S BAR=$O(^BAREDI("I",DUZ(2),"C",Z,"")) ;LIMIT TO 30 CHAR;MRS;BAR*1.8*7 IM30386  ;bar*1.8*20
