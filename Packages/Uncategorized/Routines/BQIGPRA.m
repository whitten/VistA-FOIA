...
AGG(DATA,OWNR,PLIEN) ; EP -- BQI GET GPRA AGGREGATE
 ...
 S @DATA@(II)="T00025REPORT_PERIOD^I00010TOTAL_PATIENTS^T00030CATEGORY^T00030CLIN_GROUP^I00010MEAS_IEN^T00010NATIONAL_CURRENT^T00010YEAR_CURRENT^T00040INDICATOR^I00010NUMERATOR^I00010DENOMINATOR^N00010PERCENT"_$C(30)
 ;
...
DEF(GDATA,GNUM,GDEN) ;
 ...
 S NCURR=$P(GDATA,"^",6)
 S YCURR=$P(GDATA,"^",7)
 S RPERIOD=$P(GDATA,"^",8) S:RPERIOD="" RPERIOD=$G(TPERIOD)
 S VALUE=$P(GDATA,"^",9)
 S NAFLG=$P(GDATA,"^",10)
 ;
 S NUM=+$G(GNUM)
 ...
 I NAFLG=1 S DEN="-"
 S II=II+1,@DATA@(II)=RPERIOD_"^"_TOTP_"^"_BQICAT_"^"_BQICLN_"^"_BQMEAS_"^"_NCURR_"^"_YCURR_"^"_MDESC_"^"_NUM_"^"_DEN_"^"_PER_$C(30)
 Q
 ;
RPT(VER) ;  Get the CRS Clinical Performance information
 ...
 . ;
 . I VER<8.0 D
 .. S ORDER=$$GET1^DIQ(BQIMEASF,BMEAS_",",1406,"E"),SUM="NA"
 .. I ORDER="" S ORDER=$$GET1^DIQ(BQIMEASF,BMEAS_",",1506,"E"),SUM="NN"
 ...
 .. ;S @TDATA@(SUM,ORDER,MSIEN,"NUM")=$G(@TDATA@(SUM,ORDER,MSIEN,"NUM"))+NUM
 .. ;S @TDATA@(SUM,ORDER,MSIEN,"DEN")=$G(@TDATA@(SUM,ORDER,MSIEN,"DEN"))+DEN
 .. ;S $P(@TDATA@(SUM,ORDER,MSIEN),U,8)=RPERIOD S:RPERIOD'="" TPERIOD=RPERIOD
 .. ;S $P(@TDATA@(SUM,ORDER,MSIEN),U,9)=VALUE
 . ;
 . I VER>7.0 D
...
INIT(VER,MSIEN) ; Initialize array
 ...
 ;
 I VER<8.0 D
 . S PIND=$O(^BQI(90508,BQIH,20,BQIY,20,"B",BQMEAS,""))
 . S NAFLG=+$P(^BQI(90508,BQIH,20,BQIY,20,PIND,0),U,4)
 . ;S TITLE=$P(^BQI(90506.1,MSIEN,0),U,3)
 ...
 ;
 I VER<8.0 D
 . S @TDATA@(CAT,CLIN,TITLE,MSIEN)=CAT_U_CLIN_U_MSIEN_U_TITLE_U_NCURR_U_YCURR_U_RPERIOD_U_VALUE_U_NAFLG
 . ;S @TDATA@(SUM,ORDER,MSIEN)=CAT_U_CLIN_U_MSIEN_U_TITLE_U_TWTEN_U_NCURR_U_YCURR_U_RPERIOD_U_VALUE_U_NAFLG
 I VER>7.0 D
 . S @TDATA@(CAT,CLIN,TITLE,MSIEN)=CAT_U_CLIN_U_MSIEN_U_TITLE_U_NCURR_U_YCURR_U_RPERIOD_U_VALUE_U_NAFLG
 Q
