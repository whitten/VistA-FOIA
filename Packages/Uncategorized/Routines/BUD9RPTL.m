BUD9RPTL ; IHS/CMI/LAB - UDS print lists 12 Dec 2007 12:12 PM 02 Dec 2009 10:43 AM ;
...
T3A ;
 S BUDP=0
 D T3H Q:BUDQUIT
 S BUDAGE="" F  S BUDAGE=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"3A",BUDAGE)) Q:BUDAGE'=+BUDAGE!(BUDQUIT)  D
 .S BUDSEX="" F  S BUDSEX=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX)) Q:BUDSEX=""!(BUDQUIT)  D
 ..S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ...S DFN="" F  S DFN=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX,BUDCOM,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ...
 ....S BUDV=0 F  S BUDV=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX,BUDCOM,DFN,BUDV)) Q:BUDV'=+BUDV!(BUDQUIT)  D
 .....I $Y>(IOSL-3) D T3H Q:BUDQUIT
...
T51 ;
 S BUDNEWR=1
 S BUD9L="" F  S BUD9L=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"T51",BUD9L)) Q:BUD9L=""!(BUDQUIT)  D
 .S BUD9L2="" F  S BUD9L2=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"T51",BUD9L,BUD9L2)) Q:BUD9L2=""!(BUDQUIT)  D
 ..S BUD9LL=BUD9L_$S(BUD9L2=0:"",1:BUD9L2)
 ..S BUDY=$O(^BUDNTFIV("B",BUD9LL,0)),BUDY=$P(^BUDNTFIV(BUDY,0),U,2)_" "_$P(^BUDNTFIV(BUDY,0),U,3)_" "_$P(^BUDNTFIV(BUDY,0),U,4)
 ..S BUDSUBT="Line "_BUD9LL_"   "_BUDY
 ..I $Y>(IOSL-3)!$G(BUDNEWR) D T51H Q:BUDQUIT  K BUDNEWR
 ..W !!,"Line ",BUD9LL,"   ",BUDY
 ..S BUDPROV="" F  S BUDPROV=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"T51",BUD9L,BUD9L2,BUDPROV)) Q:BUDPROV=""!(BUDQUIT)  D
 ...I $Y>(IOSL-3) D T51H Q:BUDQUIT  W !!,"Line ",BUD9LL,"   ",BUDY
 ...W !,BUDPROV,?35,^XTMP("BUD9RPT1",BUDJ,BUDH,"T51",BUD9L,BUD9L2,BUDPROV)
 .Q
 W !
 Q
T51H ;
...
T5 ;
 S BUDP=0
 S BUD9L="" F   S BUD9L=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"T5",BUD9L)) Q:BUD9L=""!(BUDQUIT)  D
 .S BUD9L2="" F  S BUD9L2=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"T5",BUD9L,BUD9L2)) Q:BUD9L2=""!(BUDQUIT)  D
 ..S BUD9LL=BUD9L_$S(BUD9L2=0:"",1:BUD9L2)
 ..S BUDY=$O(^BUDNTFIV("B",BUD9LL,0)),BUDY=$P(^BUDNTFIV(BUDY,0),U,2)_" "_$P(^BUDNTFIV(BUDY,0),U,3)_" "_$P(^BUDNTFIV(BUDY,0),U,4)
 ..S BUDSUBT="Line "_BUD9LL_"   "_BUDY
 ..D T5H Q:BUDQUIT
 ..S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"T5",BUD9L,BUD9L2,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ...S BUDAGE="" F  S BUDAGE=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"T5",BUD9L,BUD9L2,BUDCOM,BUDAGE)) Q:BUDAGE=""!(BUDQUIT)  D
 ....S BUDSEX="" F  S BUDSEX=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"T5",BUD9L,BUD9L2,BUDCOM,BUDAGE,BUDSEX)) Q:BUDSEX=""!(BUDQUIT)  D
 .....S DFN=0 F  S DFN=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"T5",BUD9L,BUD9L2,BUDCOM,BUDAGE,BUDSEX,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D T5W
 .....Q
 ...Q
 ..Q
 .Q
...
T5W W !,$E($P(^DPT(DFN,0),U,1),1,22),?24,$S($$HRN^AUPNPAT(DFN,BUDSITE)]"":$$HRN^AUPNPAT(DFN,BUDSITE,2),1:$$HRN^AUPNPAT(DFN,DUZ(2),2))
 W ?36,$E(BUDCOM,1,12),?51,$P(^DPT(DFN,0),U,2),?55,$$AGE^AUPNPAT(DFN,BUDCAD),?60,$E($P($$RACE^BUD9RPTC(DFN),U,4),1,16)," (",$P($$RACE^BUD9RPTC(DFN),U,3),")"
 K BUDVLST S BUDV=0 F  S BUDV=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"T5",BUD9L,BUD9L2,BUDCOM,BUDAGE,BUDSEX,DFN,BUDV)) Q:BUDV'=+BUDV!(BUDQUIT)  D
...
TZ ;
 S BUDP=0
 D TZH Q:BUDQUIT
 S BUDZIP="" F  S BUDZIP=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"Z",BUDZIP)) Q:BUDZIP=""!(BUDQUIT)  D
 .S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"Z",BUDZIP,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ..S BUDSEX="" F  S BUDSEX=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"Z",BUDZIP,BUDCOM,BUDSEX)) Q:BUDSEX=""!(BUDQUIT)  D
 ...S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"Z",BUDZIP,BUDCOM,BUDSEX,BUDNAME)) Q:BUDNAME=""!(BUDQUIT)  D
 ....S DFN="" F  S DFN=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"Z",BUDZIP,BUDCOM,BUDSEX,BUDNAME,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ...
 .....S BUDV=0 F  S BUDV=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"Z",BUDZIP,BUDCOM,BUDSEX,BUDNAME,DFN,BUDV)) Q:BUDV'=+BUDV!(BUDQUIT)  D
 ......I $Y>(IOSL-3) D TZH Q:BUDQUIT
 ......W !?5,$$FMTE^XLFDT($P($P(^AUPNVSIT(BUDV,0),U),".")),?25,$E($$PRIMPROV^APCLV(BUDV,"E"),1,14),?42,$P(^AUPNVSIT(BUDV,0),U,7),?45,$E($$CLINIC^APCLV(BUDV,"E"),1,14),?62,$E($$LOCENC^APCLV(BUDV,"E"),1,14)
 W !
 Q
...
T6 ;
 S BUDP=0
 F BUD9L=1:1:14,60,15:1:19,61,40:1:43,21:1:24,65,66,25,26,62,63,64,27:1:34 D
 .Q:'$D(^XTMP("BUD9RPT1",BUDJ,BUDH,"T6",BUD9L))
 .D T6H Q:BUDQUIT
 .W !!,"Line ",$S($P($T(@BUD9L),";;",3)]"":$P($T(@BUD9L),";;",3),1:BUD9L),"   ",$P($T(@BUD9L),";;",2)
 .S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"T6",BUD9L,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ..S BUDAGE="" F  S BUDAGE=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"T6",BUD9L,BUDCOM,BUDAGE)) Q:BUDAGE=""!(BUDQUIT)  D
 ...S BUDSEX="" F  S BUDSEX=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"T6",BUD9L,BUDCOM,BUDAGE,BUDSEX)) Q:BUDSEX=""!(BUDQUIT)  D
 ....S DFN=0 F  S DFN=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"T6",BUD9L,BUDCOM,BUDAGE,BUDSEX,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D T6W
 ....Q
 ...Q
 ..Q
 .Q
 ...
 Q
T6W ;
 I $Y>(IOSL-3) D T6H Q:BUDQUIT  W !!,"Line ",$S($P($T(@BUD9L),";;",3)]"":$P($T(@BUD9L),";;",3),1:BUD9L),"   ",$P($T(@BUD9L),";;",2)
 W !,$E($P(^DPT(DFN,0),U,1),1,22),?24,$S($$HRN^AUPNPAT(DFN,BUDSITE)]"":$$HRN^AUPNPAT(DFN,BUDSITE,2),1:$$HRN^AUPNPAT(DFN,DUZ(2),2))
 W ?36,$E(BUDCOM,1,12),?51,$P(^DPT(DFN,0),U,2),?55,$$AGE^AUPNPAT(DFN,BUDCAD),?60,$E($P($$RACE^BUD9RPTC(DFN),U,4),1,16)," (",$P($$RACE^BUD9RPTC(DFN),U,3),")"
 K BUDVRR S BUDV=0,BUDVC=0 F  S BUDV=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"T6",BUD9L,BUDCOM,BUDAGE,BUDSEX,DFN,BUDV)) Q:BUDV'=+BUDV!(BUDQUIT)  S BUDVC=BUDVC+1,BUDVRR($$VD^APCLV(BUDV),BUDVC)=BUDV
 S BUDVD=0 F  S BUDVD=$O(BUDVRR(BUDVD)) Q:BUDVD=""  S BUDVC=0 F  S BUDVC=$O(BUDVRR(BUDVD,BUDVC)) Q:BUDVC=""  D
 .I $Y>(IOSL-3) D T6H Q:BUDQUIT  W !!,"Line ",$S($P($T(@BUD9L),";;",3)]"":$P($T(@BUD9L),";;",3),1:BUD9L),"   ",$P($T(@BUD9L),";;",2)
 .S BUDV=BUDVRR(BUDVD,BUDVC)
 .W !?5,$$FMTE^XLFDT($P($P(^AUPNVSIT(BUDV,0),U),".")),?25,^XTMP("BUD9RPT1",BUDJ,BUDH,"T6",BUD9L,BUDCOM,BUDAGE,BUDSEX,DFN,BUDV),?40,$P(^AUPNVSIT(BUDV,0),U,7),?45,$E($$CLINIC^APCLV(BUDV,"E"),1,15),?62,$E($$LOCENC^APCLV(BUDV,"E"),1,15)
 .Q
 I BUD9L=22 D
 .S BUDW=0 F  S BUDW=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"T6",BUD9L,BUDCOM,BUDAGE,BUDSEX,DFN,"WH","MAM",BUDW)) Q:BUDW'=+BUDW!(BUDQUIT)  D
 ..W !?5,$P(^XTMP("BUD9RPT1",BUDJ,BUDH,"T6",BUD9L,BUDCOM,BUDAGE,BUDSEX,DFN,"WH","MAM",BUDW),U,2),?25,$P(^(BUDW),U,1)
 I BUD9L=23 D
 .S BUDW=0 F  S BUDW=$O(^XTMP("BUD9RPT1",BUDJ,BUDH,"T6",BUD9L,BUDCOM,BUDAGE,BUDSEX,DFN,"WH","PAP",BUDW)) Q:BUDW'=+BUDW!(BUDQUIT)  D
 ..W !?5,$P(^XTMP("BUD9RPT1",BUDJ,BUDH,"T6",BUD9L,BUDCOM,BUDAGE,BUDSEX,DFN,"WH","PAP",BUDW),U,2),?25,$P(^(BUDW),U,1)
 Q
T6H ;
 G:'BUDGPG T6H1
 K DIR I $E(IOST)="C",IO=IO(0),'$D(ZTQUEUED) W ! S DIR(0)="EO" D ^DIR K DIR I Y=0!(Y="^")!($D(DTOUT)) S BUDQUIT=1 Q
...
64 ;;Smoke/tobacco counseling; Smoking cessation treatment;;26c
