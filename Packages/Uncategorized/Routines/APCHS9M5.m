APCHS9M5 ; IHS/CMI/LAB - MEDICATION TURN AROUND SUPPLEMENT ; 
...
EP ;EP - called from component
 ...
 I $E(IOST)="C",IO=IO(0) W !! S DIR("A")="CHONIC MEDICATION RE-ORDER DOCUMENT (^ TO EXIT, RETURN TO CONTINUE)",DIR(0)="E" D ^DIR K DIR I $D(DIRUT) S APCHSQIT=1 Q  ;IHS/CMI/LAB fixed for slave printing
WR ;write out array
 K APCHQUIT
...
HEAD1 ;
 ...
 ;W !,"Medications dispensed since ",APCHSMSD,!
 Q
EP2 ;PEP - PASS DFN get back array of patient care summary
 ...
 K ^TMP($J,"APCHMEDSA")
 K ^TMP($J,"APCHMEDSG")
 S APCHSDLM=$P(^APCHSCTL(APCHSTYP,12,APCHSFOR,0),U,3)
...
SETARRAY ;set up array containing dm care summary
 ...
 Q
GETMEDS ;
 ;gather up in ^TMP all meds in past 90 DAYS that are current or Chronic
 S APCHSTOP=$$FMADD^XLFDT(APCHSMSI,-1030)
 S APCHSTOP=9999999-APCHSTOP
 ;LIMIT OF TIME ON MEDS
 ...
 I 'APCHMIEN W !!!,"No Chronic Medication Data to Display",!!
 I APCHMIEN W !!!,"Refill all meds ____ times, or ____ until next scheduled evaluation",!!,"SIGNATURE: _______________________________  DATE: ________________",!
 K ^TMP($J,"APCHMEDS")
 K ^TMP($J,"APCHCOUNT")
 K ^TMP($J,"APCHMEDSA")
 K ^TMP($J,"APCHMEDSG")
...
MEDDSP1 ;;D ^BOMB
 S APCHSM0=^AUPNVMED(APCHMIEN,0)
 S APCHSM11=$G(^AUPNVMED(APCHMIEN,11))
 ...
 D REF
 I APCHSREF W "     " S APCHSREF=APCHSREF_$S(APCHSREF=1:" refill",1:" refills")_" remaining."
 I 'APCHSREF W "     " S APCHSREF=$P($G(APCHSM11),U,7) S APCHSREF=APCHSREF_$S(APCHSREF=1:" refill",1:" refills")_" remaining"  ;CMI/GRL
 S APCHSITE=$P(^AUPNVSIT(APCHVIEN,0),U,6)
 S APCHNMBR=$P($$VALI^XBDIQ1(9000010.14,APCHMIEN,1102),"_")  ;CMI/GRL Prescription number from V Med File
 I '$G(APCHNMBR) D RXN  ;CMI/GRL check File 52 for Rx number if not in V Med
 D PROV
 D CLN
 D SITE
 D WRITE
...
WRITE ;
 ...
 ;S X=$$DATE(APCHSDTM),$E(X,10)=APCHSNAM_"  "_APCHSEXP_$S(+APCHSREF>0:"       "_APCHSREF,1:"") D W(X,1)  ;CMI/GRL
 ...
 S X="",$E(X,10)="Rx # "_$S($G(APCHNMBR)]"":APCHNMBR,1:""),$E(X,30)=$S(+APCHSREF>0:"       "_APCHSREF,1:"")  D W(X)  ;CMI;GRL
 ;ISSUE HISTORY
 ...
 ;I X]"" D W(X)  ;CMI/GRL suppress
 ;I APCHSREF S X="",$E(X,10)=APCHSREF D W(X)  ;cmi/grl
 S X=""
...
REF ;
 ...
 S APCHSREF=APCHSRFL
 Q
RXN ;Get Rx number from File 52 if not in V Med File
 I 'APCHSRX S APCHNMBR="None on file" Q
 S APCHNMBR=$P($G(^PSRX(APCHSRX,0)),U)
 I '$G(APCHNMBR) S APCHNMBR=" None on file"
 Q
 ;
