BUD8RPTP ; IHS/CMI/LAB - UDS REPORT PRINT 30 Dec 2008 10:44 AM ;
...
T3A ;
 ...
 ;print out each line
 S BUDZ=0 F  S BUDZ=$O(^BUDGTTA("AC",BUDZ)) Q:BUDZ>38!(BUDQUIT)  D
 .S BUDC=$O(^BUDGTTA("AC",BUDZ,0))
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T3ASH
 .S BUDY=^BUDGTTA(BUDC,0)
 ...
 S BUDC=$O(^BUDGTTA("B",43,0))
 I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T3ASH
 W !,$P(^BUDGTTA(BUDC,0),U,2),?5,$P(^BUDGTTA(BUDC,0),U,3)
 S BUDC=$O(^BUDGTTA("AC",39,0))
 ...
 Q
T3ASH ;
 W !,$$CTR($G(^BUDGTTA(1,11)),80),!
 F Y=2:1:4 S X=$O(^BUDGTTA("B",Y,0)) W !,$P(^BUDGTTA(X,0),U,2),?5,$P(^BUDGTTA(X,0),U,3),?33,$P(^BUDGTTA(X,0),U,4),?50,$P(^BUDGTTA(X,0),U,5),?68,$P(^BUDGTTA(X,0),U,6)
 W !,$TR($J("",80)," ","-")
 Q
T3BSH ;
 W !!,$$CTR("TABLE 3B -",80)
 W !,$$CTR("PATIENTS BY ETHNICITY/RACE/LANGUAGE",80),!,BUD80L
 ;W !,$TR($J("",80)," ","-")
 Q
TZSH ;
...
T3B ;
 ...
 ;print out each line
 W !?66,"NUMBER"
 W !,"PATIENTS BY ETHNICITY",?67,"(a)",!,BUD80L
 ...
 W !,"NUMBER OF PATIENTS",!,BUD80L
 F BUDX="5B","5C","5Z","5A",6,7,8,9,10,11 Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T3BSH
...
TZ ;
 ...
 ;print out each line
 S BUDZTOT=0
 S ^XTMP("BUD8RPT1",BUDJ,BUDH,"ZIP","OTHER ZIP CODES")=0
 ;GET TOTAL
 S BUDZ="" F  S BUDZ=$O(^XTMP("BUD8RPT1",BUDJ,BUDH,"ZIP",BUDZ)) Q:BUDZ=""  D
 .S BUDY=^XTMP("BUD8RPT1",BUDJ,BUDH,"ZIP",BUDZ)
 ...
 .Q:BUDZ["Unknown"
 .S BUDY=^XTMP("BUD8RPT1",BUDJ,BUDH,"ZIP",BUDZ)
 .S X=BUDY/BUDZTOT I X<.001 K ^XTMP("BUD8RPT1",BUDJ,BUDH,"ZIP",BUDZ) S ^XTMP("BUD8RPT1",BUDJ,BUDH,"ZIP","OTHER ZIP CODES")=^XTMP("BUD8RPT1",BUDJ,BUDH,"ZIP","OTHER ZIP CODES")+BUDY D
 ..;now reset the list so the others collate at the end
 ..S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUD8RPT1",BUDJ,BUDH,"Z",BUDZ,BUDCOM)) Q:BUDCOM=""  D
 ...S BUDSEX="" F  S BUDSEX=$O(^XTMP("BUD8RPT1",BUDJ,BUDH,"Z",BUDZ,BUDCOM,BUDSEX)) Q:BUDSEX=""  D
 ....S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUD8RPT1",BUDJ,BUDH,"Z",BUDZ,BUDCOM,BUDSEX,BUDNAME)) Q:BUDNAME=""  D
 .....S DFN=0 F  S DFN=$O(^XTMP("BUD8RPT1",BUDJ,BUDH,"Z",BUDZ,BUDCOM,BUDSEX,BUDNAME,DFN)) Q:DFN'=+DFN  D
 ......S X=0 F  S X=$O(^XTMP("BUD8RPT1",BUDJ,BUDH,"Z",BUDZ,BUDCOM,BUDSEX,BUDNAME,DFN,X)) Q:X'=+X  D
 .......S ^XTMP("BUD8RPT1",BUDJ,BUDH,"Z",BUDZ_"*",BUDCOM,BUDSEX,BUDNAME,DFN,X)=""
 .......K ^XTMP("BUD8RPT1",BUDJ,BUDH,"Z",BUDZ,BUDCOM,BUDSEX,BUDNAME,DFN,X)
  S BUDZ="" F  S BUDZ=$O(^XTMP("BUD8RPT1",BUDJ,BUDH,"ZIP",BUDZ)) Q:BUDZ=""!(BUDQUIT)  D
...
T5 ;print table 5
 ...
 ;lines 1-8
 F BUDL=1:1:7 S BUDY=$O(^BUDGTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .Q:'BUDY
 .W !
 .I +BUDL<10 W " "
 .W BUDL,".",?6,$P(^BUDGTFIV(BUDY,0),U,2)
 .I $P(^BUDGTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDGTFIV(BUDY,0),U,3)
 ...
 F BUDL="9A","9B",10 S BUDY=$O(^BUDGTFIV("B",BUDL,0)) Q:BUDQUIT  D
 ...
 F BUDL=11:1:14 S BUDY=$O(^BUDGTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDGTFIV(BUDY,0),U,2) I $P(^BUDGTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDGTFIV(BUDY,0),U,3)
 ...
 F BUDL=16:1:17 S BUDY=$O(^BUDGTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDGTFIV(BUDY,0),U,2) I $P(^BUDGTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDGTFIV(BUDY,0),U,3)
 .W ?60,$$C($P(BUDTAB5(BUDL),U,1),0)
 .W ?71,$$C($P(BUDTAB5(BUDL),U,2),2)
 .W !,BUD80L
T518 F BUDL=18 S BUDY=$O(^BUDGTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDGTFIV(BUDY,0),U,2) I $P(^BUDGTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDGTFIV(BUDY,0),U,3)
 ...
 F BUDL="20A","20A1","20A2","20B","20C" S BUDY=$O(^BUDGTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDGTFIV(BUDY,0),U,2)
 .I $P(^BUDGTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDGTFIV(BUDY,0),U,3)
 .I $P(^BUDGTFIV(BUDY,0),U,4)]"" W !?6,$P(^BUDGTFIV(BUDY,0),U,4)
 .W ?60,$$C($P(BUDTAB5(BUDL),U,1),0)
 .W ?71,$$C($P(BUDTAB5(BUDL),U,2),2)
 .W !,BUD80L
 I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 W !,"20.",?10,"Mental Health Services",!?10,"(Lines 20a - c)",?60,$$C($P(BUDTAB5(20),U,1)),?71,$$C($P(BUDTAB5(20),U,2)),!,BUD80L
 F BUDL=21:1:22 S BUDY=$O(^BUDGTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDGTFIV(BUDY,0),U,2) I $P(^BUDGTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDGTFIV(BUDY,0),U,3)
 .W ?60,$$C($P(BUDTAB5(BUDL),U,1),0)
 .W ?71,$$C($P(BUDTAB5(BUDL),U,2),0)
 .W !,BUD80L
 F BUDL=23 S BUDY=$O(^BUDGTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDGTFIV(BUDY,0),U,2) I $P(^BUDGTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDGTFIV(BUDY,0),U,3)
 .W ?60,$$C($P(BUDTAB5(BUDL),U,1),2)
 .W ?71,$$C($P(BUDTAB5(BUDL),U,2),2)
 .W !,BUD80L
 F BUDL=24:1:25 S BUDY=$O(^BUDGTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDGTFIV(BUDY,0),U,2) I $P(^BUDGTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDGTFIV(BUDY,0),U,3)
 .W ?60,$$C($P(BUDTAB5(BUDL),U,1),0)
 .W ?71,$$C($P(BUDTAB5(BUDL),U,2),2)
 .W !,BUD80L
 F BUDL=26,27,"27a","27b",28 S BUDY=$O(^BUDGTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDGTFIV(BUDY,0),U,2) I $P(^BUDGTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDGTFIV(BUDY,0),U,3)
 ...
 F BUDL="29A","30A","30B","30C" S BUDY=$O(^BUDGTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDGTFIV(BUDY,0),U,2) I $P(^BUDGTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDGTFIV(BUDY,0),U,3)
 .W ?60,$$C($P(BUDTAB5(BUDL),U,1),2)
 .W ?71,$$C($P(BUDTAB5(BUDL),U,2),2)
 ...
 I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
  F BUDL=31,32 S BUDY=$O(^BUDGTFIV("B",BUDL,0)) Q:BUDQUIT  D
 .I $Y>(IOSL-3) D HEADER Q:BUDQUIT  D T5SH
 .W !,$$LOW^XLFSTR(BUDL),".",?6,$P(^BUDGTFIV(BUDY,0),U,2) I $P(^BUDGTFIV(BUDY,0),U,3)]"" W !?6,$P(^BUDGTFIV(BUDY,0),U,3)
 .W ?60,$$C($P(BUDTAB5(BUDL),U,1),2)
 .W ?71,$$C($P(BUDTAB5(BUDL),U,2),2)
...
HEADER1 ;
 ...
 S BUDPG=BUDPG+1
 S X=$$CTR($P(^DIC(4,BUDSITE,0),U),60),$E(X,3)=$P(^VA(200,DUZ,0),U,2),$E(X,10)="UDS 2008",$E(X,70)="Page ",$E(X,75)=BUDPG W !,X
 ;W !?3,$P(^VA(200,DUZ,0),U,2),?10,"UDS 2008",$$CTR($P(^DIC(4,BUDSITE,0),U),80),?70,"Page ",BUDPG
 ;W $$CTR($P(^DIC(4,BUDSITE,0),U),80),!
 W !,"UDS No.  ",$P(^BUDGSITE(BUDSITE,0),U,2),?50,"Date Run: ",$$FMTE^XLFDT(DT)
