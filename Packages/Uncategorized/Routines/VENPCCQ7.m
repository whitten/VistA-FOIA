...
POSTINIT ; ------------------- EPs FOR POST INIT ------------------------
 ; 
KBM(START,END) ; EP - GIVEN START AND END IENs, ADD ITEMS TO VEN EHP KB ITEMS FILE FROM THE VEN EHP KB MASTER FILE ; TARGET SERVER
 I $D(^VEN(7.12)),$D(START),$G(END)
 E  W !!!!,"KB file missing!  Post init terminated...",!!! Q
...
CKVOBJ(BLD,OK) ; EP - CHECK REGISTERED VUECENTRIC OBJECTS ; TARGET SERVER
 ...
 S NM=""
 F  S NM=$O(@VGBL@(BIEN,1,"B",NM)) Q:NM=""  D  ; CHECK OBJECTS AND BUILD THE OBJECT IX ARRAY
 ...
 .. Q
 . S IX(IEN)=NAME,DNM=""
 . F  S DNM=$O(@VGBL@(BIEN,1,VIEN,1,"B",DNM)) Q:DNM=""  D
 .. S DIEN=$O(@VGBL@(BIEN,1,VIEN,1,"B",DNM,0)) I 'DIEN Q
...
DEP F  S IEN=$O(IX(IEN)) Q:'IEN  D  ; REBUILD THE SUBFILE WITH IEN FROM THE TARGET MACHINE
 ...
 W !?7,"All VueCentric Objects and dependencies have been successfully entered",!
 Q
 ;
...
CIABMX(OK) ; EP - IF NECESSARY, ADD THE RPC CIABMX TO THE CIAV VUECENTRIC OPTION
 ...
 S RIEN=$O(^XWB(8994,"B","CIABMX",0)) I 'RIEN W !?5,"The RPC 'CIABMX' is missing",! S OK=1 Q
 S OIEN=$O(^DIC(19,"B","CIAV VUECENTRIC",0)) I 'OIEN W !?5,"The Option 'CIAV VUECENTRIC' is missing",! S OK=1 Q
