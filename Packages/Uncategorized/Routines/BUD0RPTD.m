...
GETV ;EP - get all visits for this patient and tally in BUDTV
 ...
 .Q:BUDVLOC=""
 .Q:'$D(^BUDTSITE(BUDSITE,11,BUDVLOC))  ;not valid location
 .Q:"AHSORMEI"'[$P(^AUPNVSIT(BUDVSIT,0),U,7)
 .S BUDCLINC=$$CLINIC^APCLV(BUDVSIT,"C")
 .S BUDTIEN=$O(^BUDTCNTL("B","FIRST LEVEL CLINIC EXCLUSIONS",0))
 .I BUDCLINC]"",$D(^BUDTCNTL(BUDTIEN,11,"B",BUDCLINC)) Q  ;not a clinic code we want in any table
...
MEDC .;NOW CHECK FOR MEDICAL CARE, CAN ONLY HAVE 1 PER LOCATION OF ENCOUNTER
 .S S=0
 .S BUDTIEN=$O(^BUDTCNTL("B","MEDICAL CARE LINE NUMBERS",0))
 .;S BUDPP=$$PRIMPROV^APCLV(BUDVSIT,"D")
 .I $E($$VAL^XBDIQ1(9000010,BUDVSIT,.06),1,3)="CHS",BUDPP=15 S BUDLINE=2 G MEDC1
 .S BUDY=$O(^BUDTTFIV("C",BUDPP,0)) I BUDY="" S BUDLINE=35 G MEDC1
 .S BUDLINE=$O(^BUDTTFIV("AA",BUDPP,""))
MEDC1 .S S=0
 .I $D(^BUDTCNTL(BUDTIEN,11,"B",BUDLINE)) D
...
DENT .;NOW CHECK FOR DENTAL CARE
 .S S=0
 .S BUDTIEN=$O(^BUDTCNTL("B","DENTAL LINE NUMBERS",0))
 .S S=0
 .I $D(^BUDTCNTL(BUDTIEN,11,"B",BUDLINE)) D
...
MH .;NOW CHECK FOR MH CARE
 .S S=0
 .S BUDTIEN=$O(^BUDTCNTL("B","MENTAL HEALTH LINE NUMBERS",0))
...
SUB .;
 .S S=0
 .S BUDTIEN=$O(^BUDTCNTL("B","SUBSTANCE LINE NUMBERS",0))
 .S S=0
 .I $D(^BUDTCNTL(BUDTIEN,11,"B",BUDLINE)) D
...
VISION .;
 .S S=0
 .S BUDTIEN=$O(^BUDTCNTL("B","VISION LINE NUMBERS",0))
 .S S=0
 .I $D(^BUDTCNTL(BUDTIEN,11,"B",BUDLINE)) D
...
OTH .;can have only 1 in each category
 .S BUDTIEN=$O(^BUDTTFIV("B",22,0))
 .S S=0
 .I $D(^BUDTTFIV(BUDTIEN,11,"B",BUDPP)) D
...
ENAB .;NOW CHECK FOR ENABLING
 .S S=0
 .S BUDTIEN=$O(^BUDTCNTL("B","ENABLING LINE NUMBERS",0))
 .S S=0
 .I $D(^BUDTCNTL(BUDTIEN,11,"B",BUDLINE)) D
...
SET .S BUDTV=BUDTV+1
 ...
 .S BUDTIEN=$O(^BUDTCNTL("B","CLINIC EXCLUSIONS",0))
 .I BUDCLINC]"",$D(^BUDTCNTL(BUDTIEN,11,"B",BUDCLINC)) Q  ;exclude these clinics
 ...
 ...Q:L=""
 ...Q:'$D(^BUDTSITE(BUDSITE,11,L))  ;not valid location
