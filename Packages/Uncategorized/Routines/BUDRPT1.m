BUDRPT1 ; IHS/CMI/LAB - UDS REPORT DRIVER ;
...
EN1 ;
 S BUDSITE=""
 S DIC="^BUDUSITE(",DIC(0)="AEMQ",DIC("A")="Enter your site: " D ^DIC
 I Y=-1 D EOJ Q
 S BUDSITE=+Y
 I '$O(^BUDUSITE(BUDSITE,11,0)) W !!,"Warning:  There are no locations defined in the site parameter file for this",!,"site.  Report will not be accurate!" G EN
 D TAXCHK^BUDTXCH
 D YEAR
 I BUDYEAR="" W !!,"Year not entered.",! D EOJ Q
 D QUARTER  ;get time period
 I BUDQTR="" W !,"Time period not entered." D EOJ Q
 ;S XBRP="PRINT^BUDRPTP",XBRC="PROC^BUDRPTC",XBRX="EOJ^BUDRPT1",XBNS="BUD"
 ;D ^XBDBQUE
 ;D EOJ
 ;Q
ZIS ;call to XBDBQUE
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BUDRPT1",ZTDTH="",ZTDESC="UDS 03 REPORT" D ^%ZTLOAD D EOJ Q
 Q
...
GENI ;general introductions
 ...
 W !,"during the time period selected. Based on the UDS definition, visits must meet",!,"the following criteria:"
 W !?4,"- must be to a location specified in your visit location setup"
 W !?4,"- must be to Service Category Ambulatory (A), Hospitalization (H), Day"
 W !?6,"Surgery (S), Observation (O), Nursing home visit (R) or Historical Event",!?6,"(E) visit"
