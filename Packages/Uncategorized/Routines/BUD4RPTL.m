...
T3A ;
 S BUDP=0
 D T3H Q:BUDQUIT
 S BUDAGE="" F  S BUDAGE=$O(^XTMP("BUD4RPT1",BUDJ,BUDH,"3A",BUDAGE)) Q:BUDAGE'=+BUDAGE!(BUDQUIT)  D
 .S BUDSEX="" F  S BUDSEX=$O(^XTMP("BUD4RPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX)) Q:BUDSEX=""!(BUDQUIT)  D
 ..S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUD4RPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ...S DFN="" F  S DFN=$O(^XTMP("BUD4RPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX,BUDCOM,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ....I $Y>(IOSL-3) D T3H Q:BUDQUIT
 ....W !,$E($P(^DPT(DFN,0),U,1),1,22),?24,$S($$HRN^AUPNPAT(DFN,BUDSITE)]"":$$HRN^AUPNPAT(DFN,BUDSITE,2),1:$$HRN^AUPNPAT(DFN,DUZ(2),2)),?36,$E(BUDCOM,1,12),?51,$P(^DPT(DFN,0),U,2),?55,$$AGE^AUPNPAT(DFN,BUDCAD),?60,$P($$RACE^BUD4RPTC(DFN),U,2)
 ....S BUDV=0 F  S BUDV=$O(^XTMP("BUD4RPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX,BUDCOM,DFN,BUDV)) Q:BUDV'=+BUDV!(BUDQUIT)  D
 .....I $Y>(IOSL-3) D T3H Q:BUDQUIT
...
T51 ;
 D T51H Q:BUDQUIT
 S BUD5L="" F  S BUD5L=$O(^XTMP("BUD4RPT1",BUDJ,BUDH,"T51",BUD5L)) Q:BUD5L=""!(BUDQUIT)  D
 .S BUD5L2="" F  S BUD5L2=$O(^XTMP("BUD4RPT1",BUDJ,BUDH,"T51",BUD5L,BUD5L2)) Q:BUD5L2=""!(BUDQUIT)  D
 ..S BUD5LL=BUD5L_$S(BUD5L2=0:"",1:BUD5L2)
 ..I $Y>(IOSL-3) D T51H Q:BUDQUIT
 ..S BUDY=$O(^BUDFTFIV("B",BUD5LL,0)),BUDY=$P(^BUDFTFIV(BUDY,0),U,2)
 ..W !!,"Line ",BUD5LL,"   ",BUDY
 ..S BUDPROV="" F  S BUDPROV=$O(^XTMP("BUD4RPT1",BUDJ,BUDH,"T51",BUD5L,BUD5L2,BUDPROV)) Q:BUDPROV=""!(BUDQUIT)  D
 ...W !,BUDPROV,?35,^XTMP("BUD4RPT1",BUDJ,BUDH,"T51",BUD5L,BUD5L2,BUDPROV)
 .Q
 Q
T51H ;
 G:'BUDGPG T51H1
...
T5 ;
 S BUDP=0
 S BUD5L="" F   S BUD5L=$O(^XTMP("BUD4RPT1",BUDJ,BUDH,"T5",BUD5L)) Q:BUD5L=""!(BUDQUIT)  D
 .S BUD5L2="" F  S BUD5L2=$O(^XTMP("BUD4RPT1",BUDJ,BUDH,"T5",BUD5L,BUD5L2)) Q:BUD5L2=""!(BUDQUIT)  D
 ..D T5H Q:BUDQUIT
 ..S BUD5LL=BUD5L_$S(BUD5L2=0:"",1:BUD5L2)
 ..S BUDY=$O(^BUDFTFIV("B",BUD5LL,0)),BUDY=$P(^BUDFTFIV(BUDY,0),U,2)
 ..W !!,"Line ",BUD5LL,"   ",BUDY
 ...
 .Q
 Q
T5W W !,$E($P(^DPT(DFN,0),U,1),1,22),?24,$S($$HRN^AUPNPAT(DFN,BUDSITE)]"":$$HRN^AUPNPAT(DFN,BUDSITE,2),1:$$HRN^AUPNPAT(DFN,DUZ(2),2)),?36,$E(BUDCOM,1,12),?51,$P(^DPT(DFN,0),U,2),?55,$$AGE^AUPNPAT(DFN,BUDCAD),?60,$P($$RACE^BUD4RPTC(DFN),U,2)
...
T6 ;
 S BUDP=0
 ;D T6H Q:BUDQUIT
 ;S BUD6L="" F   S BUD6L=$O(^XTMP("BUD4RPT1",BUDJ,BUDH,"T6",BUD6L)) Q:BUD6L=""!(BUDQUIT)  D
 F BUD6L=1:1:19,40:1:43,21:1:34 D
 .Q:'$D(^XTMP("BUD4RPT1",BUDJ,BUDH,"T6",BUD6L))
 .D T6H Q:BUDQUIT
 .W !!,"Line ",$S($P($T(@BUD6L),";;",3)]"":$P($T(@BUD6L),";;",3),1:BUD6L),"   ",$P($T(@BUD6L),";;",2)
 .S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUD4RPT1",BUDJ,BUDH,"T6",BUD6L,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ..S BUDAGE="" F  S BUDAGE=$O(^XTMP("BUD4RPT1",BUDJ,BUDH,"T6",BUD6L,BUDCOM,BUDAGE)) Q:BUDAGE=""!(BUDQUIT)  D
 ...S BUDSEX="" F  S BUDSEX=$O(^XTMP("BUD4RPT1",BUDJ,BUDH,"T6",BUD6L,BUDCOM,BUDAGE,BUDSEX)) Q:BUDSEX=""!(BUDQUIT)  D
 ....S DFN=0 F  S DFN=$O(^XTMP("BUD4RPT1",BUDJ,BUDH,"T6",BUD6L,BUDCOM,BUDAGE,BUDSEX,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D T6W
 ....Q
 ...Q
 ..Q
 .Q
...
T6W ;
 ...
 W !,$E($P(^DPT(DFN,0),U,1),1,22),?24,$S($$HRN^AUPNPAT(DFN,BUDSITE)]"":$$HRN^AUPNPAT(DFN,BUDSITE,2),1:$$HRN^AUPNPAT(DFN,DUZ(2),2)),?36,$E(BUDCOM,1,12),?51,$P(^DPT(DFN,0),U,2),?55,$$AGE^AUPNPAT(DFN,BUDCAD),?60,$P($$RACE^BUD4RPTC(DFN),U,2)
 S BUDV=0 F  S BUDV=$O(^XTMP("BUD4RPT1",BUDJ,BUDH,"T6",BUD6L,BUDCOM,BUDAGE,BUDSEX,DFN,BUDV)) Q:BUDV'=+BUDV!(BUDQUIT)  D
 .I $Y>(IOSL-3) D T6H Q:BUDQUIT
 .W !?5,$$FMTE^XLFDT($P($P(^AUPNVSIT(BUDV,0),U),".")),?25,^XTMP("BUD4RPT1",BUDJ,BUDH,"T6",BUD6L,BUDCOM,BUDAGE,BUDSEX,DFN,BUDV),?40,$P(^AUPNVSIT(BUDV,0),U,7),?45,$E($$CLINIC^APCLV(BUDV,"E"),1,15),?62,$E($$LOCENC^APCLV(BUDV,"E"),1,15)
 .Q
 I BUD6L=22 D
 .S BUDW=0 F  S BUDW=$O(^XTMP("BUD4RPT1",BUDJ,BUDH,"T6",BUD6L,BUDCOM,BUDAGE,BUDSEX,DFN,"WH","MAM",BUDW)) Q:BUDW'=+BUDW!(BUDQUIT)  D
 ..W !?5,$P(^XTMP("BUD4RPT1",BUDJ,BUDH,"T6",BUD6L,BUDCOM,BUDAGE,BUDSEX,DFN,"WH","MAM",BUDW),U,2),?25,$P(^(BUDW),U,1)
 I BUD6L=23 D
 .S BUDW=0 F  S BUDW=$O(^XTMP("BUD4RPT1",BUDJ,BUDH,"T6",BUD6L,BUDCOM,BUDAGE,BUDSEX,DFN,"WH","PAP",BUDW)) Q:BUDW'=+BUDW!(BUDQUIT)  D
 ..W !?5,$P(^XTMP("BUD4RPT1",BUDJ,BUDH,"T6",BUD6L,BUDCOM,BUDAGE,BUDSEX,DFN,"WH","PAP",BUDW),U,2),?25,$P(^(BUDW),U,1)
 Q
T6H ;
 G:'BUDGPG T6H1
 K DIR I $E(IOST)="C",IO=IO(0),'$D(ZTQUEUED) W ! S DIR(0)="EO" D ^DIR K DIR I Y=0!(Y="^")!($D(DTOUT)) S BUDQUIT=1 Q
