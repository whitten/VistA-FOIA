...
INFORM ;
 W !,$$CTR($$USR)
 W !,$$LOC()
 W !!,$$CTR("LISTING OF PATIENT REFUSALS",80)
 W !!,"This report will list all refusals documented for patients.",!,"You will be given the opportunity to specify which type of refusals and the",!,"date range of the refusals.",!
 W !
TYPE ;type of refusal all or one?
 S APCLTYPE=""
 S DIR(0)="Y",DIR("A")="Do you want to include ALL Refusal Types?",DIR("B")="Y" KILL DA D ^DIR KILL DIR
 I $D(DIRUT) G EOJ
 S APCLTYPE=Y
 I APCLTYPE G DATES
TYPE1 ;which refusal type?
 S APCLTYP1=""
 K DIC S DIC="^AUTTREFT(",DIC(0)="AEMQ",DIC("A")="Enter the refusal type: " D ^DIC K DIC
 I Y=-1 G TYPE
 S APCLTYP1=+Y
ID ;
 S APCLID="",APCLONE=""
 S DIR(0)="S^A:ALL "_$P(^AUTTREFT(APCLTYP1,0),U)_" refusals;O:ONE "_$P(^AUTTREFT(APCLTYP1,0),U)_"  refusal"
 S DIR("A")="Do you want one particular "_$P(^AUTTREFT(APCLTYP1,0),U)_"  refusal or all",DIR("B")="A" KILL DA D ^DIR KILL DIR
 I $D(DIRUT) G TYPE
 S APCLID=Y
 G:APCLID="A" DATES
...
DATES K APCLED,APCLBD
 K DIR W ! S DIR(0)="DO^::EXP",DIR("A")="Enter Beginning Refusal Date"
 D ^DIR Q:Y<1  S APCLBD=Y
 K DIR S DIR(0)="DO^:DT:EXP",DIR("A")="Enter Ending Refusal Date"
...
HEADER1 ;
 ...
 W !,$$CTR("***  PATIENT REFUSAL LISTING  ***",80),!
 S X=$S(APCLTYPE:"ALL REFUSAL TYPES",1:$P(^AUTTREFT(APCLTYP1,0),U)_" REFUSALS") W $$CTR(X,80),!
 I $G(APCLONE)]"" W $$CTR(APCLONEP_" refusals",80),!
 S X="Refusal Dates: "_$$FMTE^XLFDT(APCLBD)_" to "_$$FMTE^XLFDT(APCLED) W $$CTR(X,80),!
 W !,"PATIENT NAME",?22,"HRN",?29,"DOB",?42,"DATE REFUSED",?55,"REFUSAL",?71,"REASON"
 W !,$TR($J("",80)," ","-")
 Q
DONE ;
 K ^XTMP("APCLREF1",APCLJ,APCLH)
