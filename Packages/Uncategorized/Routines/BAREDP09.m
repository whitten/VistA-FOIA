BAREDP09 ; IHS/SD/LSL - FIND ERA CHECKS AND MATCH TO RPMS ;07/08/2008
 ...
 ; IHS/SD/LSL - 09/29/03 - V1.7 P4 - HIPAA - Routine created
 ; IHS/SD/LSL - 02/26/04 - V1.7 P5 - Change Check to chk/EFT #
 ; IHS/SD/RLT - 10/05/05 - IM13516 V1.8 P1 - Can't review ERA files/checks
 ; IHS/SD/TPF - 12/28/04 - IM15268,IM17926 V1.8 P1 - ERA check matching - don't allow cancelled/rolled up to display
 Q
CHECK(IMPDA) ; EP
 ;Select Chk to review
 ...
 ;S BARCKIEN=$$CHKSEL^BAREUTL(IMPDA,"REVIEW")  ;bar*1.8*20 REQ3
...
EXIST ;
 ...
 ;I '$D(^BARECHK("B",I)) Q  ;bar*1.*20 REQ3
 I '$D(^BARECHK("B",BARI)) Q  ;bar*1.8*20 REQ3
 S BARMTCH=0
 S BARCKIEN=0
 ;F  S BARCKIEN=$O(^BARECHK("B",I,BARCKIEN)) Q:'+BARCKIEN  D  Q:+BARMTCH  ;bar*1.8*20 REQ3
 ...
 .;start old bar*1.8*20 REQ3
 .;S $P(BARCHK(I),U,2)=BARBATCH
 .;S $P(BARCHK(I),U,3)=BARITEM
 .;end old start new REQ3
...
BATCHECK ;
 ...
 .;I $P(BARCHK(BARCHECK),U,2),$P(BARCHK(BARCHECK),U,3) D  Q  ;bar*1.8*20 REQ3
 .I $P(BARCHK(BARCHECK),U,3),$P(BARCHK(BARCHECK),U,4) D  Q  ;bar*1.8*20 REQ3
 ..;S BARBATCH=$P(BARCHK(BARCHECK),U,2)  ;bar*1.8*20 REQ3
 ..S BARBATCH=$P(BARCHK(BARCHECK),U,3)  ;bar*1.8*20 REQ3
 ..;S BARITEM=$P(BARCHK(BARCHECK),U,3)  ;bar*1.8*20 REQ3
 ...
 .S BARCNT2=0
 .;I '$D(^BARCOL(DUZ(2),"D",BARCHECK)) S $P(BARCHK(BARCHECK),U)=0,BARTMP=0 Q  ;bar*1.8*20 REQ3
 .I '$D(^BARCOL(DUZ(2),"D",BARCHKN)) S $P(BARCHK(BARCHECK),U,2)=0,BARTMP=0 Q  ;bar*1.8*20 REQ3
 .;I $D(^BARCOL(DUZ(2),"D",BARCHECK)) D  ;bar*1.8*20 REQ3
 .I $D(^BARCOL(DUZ(2),"D",BARCHKN)) D  ;bar*1.8*20 REQ3
 ..S BARBATCH=0  ;Collection Batch IEN
 ..;F  S BARBATCH=$O(^BARCOL(DUZ(2),"D",BARCHECK,BARBATCH)) Q:'+BARBATCH  D  ;bar*1.8*20 REQ3
