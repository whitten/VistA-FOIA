...
LDL ;EP - cholesterol CCI measure
 ...
 D S^APCHPWH1("")
 I APCHCHOL="" D  Q
 .D S^APCHPWH1("No recent cholesterol is on file.  You should have your cholesterol")
...
CVD(P) ;EP
 ...
 I 'T Q ""
 S (X,Y,I)=0 F  S X=$O(^AUPNPROB("AC",P,X)) Q:X'=+X!(I)  I $D(^AUPNPROB(X,0)),$P(^AUPNPROB(X,0),U,12)'="D" S Y=$P(^AUPNPROB(X,0),U) I $$ICD^ATXCHK(Y,T,9) S I=1
 I I Q 1
...
AMIO(P,EDATE) ;
 ...
 S APCHG=$$LASTPRC^APCHSMU2(P,"BGP PTCA PROCS",$$FMADD^XLFDT(EDATE,-365),$$FMADD^XLFDT(EDATE))
 I $P(APCHG,U) Q 1
 ;now check cpts
...
AIC ;a1c
 ...
 D S^APCHPWH1("Hemoglobin A1c is a test that measures your blood sugar control over a")
 D S^APCHPWH1("3-month period.  You should have this test done every 3-6 months.")
 ...
 .D S^APCHPWH1("You are due to have your A1c tested. Ask your health care provider",1)
...
KT ;kidney assessment
 ...
 I APCHKT="" D
 .D S^APCHPWH1("You should have your kidneys tested to see how well they are working every ",1)
 .D S^APCHPWH1("year. Ask your health care provider to order a kidney function test for you.")
 I APCHKT]"" D
 .D S^APCHPWH1("Your kidneys were tested on "_$$FMTE^XLFDT($P(APCHKT,U))_". You will need to have your ",1)
 .D S^APCHPWH1("kidneys tested again on "_$$FMTE^XLFDT($$FMADD^XLFDT($P(APCHKT,U),365))_".")
EYE ;EYE
 ...
 I 'APCHKT D  G FOOT
 .D S^APCHPWH1("You should have at least one diabetes eye exam every year. Ask your health ",1)
 .D S^APCHPWH1("care provider to order a diabetes eye exam for you.")
 I APCHKT,$$FMDIFF^XLFDT(DT,$P(APCHKT,U,2))<365 D  G FOOT
 .D S^APCHPWH1("Your last diabetes eye exam was done on "_$$FMTE^XLFDT($P(APCHKT,U,2))_".  You will need to",1)
 .D S^APCHPWH1("have another eye exam by "_$$FMTE^XLFDT($$FMADD^XLFDT($P(APCHKT,U,2),365))_".")
...
FOOT ;FOOT
 ...
 D S^APCHPWH1("exam every year can help to lower the chance of losing feeling in your feet,")
 D S^APCHPWH1("getting an infection, or having an amputation.")
 S APCHKT=$$FOOTEX^APCHPWH6(APCHSDFN,$$DOB^AUPNPAT(APCHSDFN),DT)
 I 'APCHKT D  Q
 .D S^APCHPWH1("You should have at least one diabetes foot exam every year. Ask your health ",1)
 .D S^APCHPWH1("care provider to order a diabetes foot exam for you.")
 I APCHKT,$$FMDIFF^XLFDT(DT,$P(APCHKT,U))<365 D  Q
 .D S^APCHPWH1("Your last diabetes foot exam was done on "_$$FMTE^XLFDT($P(APCHKT,U))_".  You will need to",1)
 .D S^APCHPWH1("have another foot exam by "_$$FMTE^XLFDT($$FMADD^XLFDT($P(APCHKT,U),365))_".")
...
EYET(P,BDATE,EDATE) ;
 ...
 I $D(APCHG(1)),$P(APCHLEYE,U,2)<$P(APCHG(1),U) S APCHLEYE="3^"_$P(APCHG(1),U)_"^V72.0 POV"
 ...
 .S X=$$CPT^APCHPWHU(P,BDATE,EDATE,T,5) I X]"" Q
 .S X=$$TRAN^APCHPWHU(P,BDATE,EDATE,T,5)
 S X=$$LASTPRCI^APCHSMU2(P,"95.02",BDATE,EDATE) I X,$P(APCHLEYE,U,2)<$P(X,U,3) S APCHLEYE=3_U_$P(X,U,3)_U_"PROC: 95.02"
 Q APCHLEYE
 ;
