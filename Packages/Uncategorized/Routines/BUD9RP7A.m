BUD9RP7A ; IHS/CMI/LAB - UDS REPORT PROCESSOR 01 Dec 2009 5:11 PM ;
...
PRGHLST ;EP - list of pregnant females
 ...
 S BUDRACEX=$$RACE^BUD9RPTC(DFN),BUDRACE=$P(BUDRACEX,U,2),BUDRACEP=$P(BUDRACEX,U,5)
 S BUDRACEE=$$RACE^BUD9RP7I(BUDRACE)
 S BUDR=""
 S BUDETHN=$P($$HISP^BUD9RPTC(DFN),U,1)
 ...
 I BUDRACEP=16!(+BUDETHN=3) S BUDR=16 G SETSECTH
 I +BUDETHN=1 S BUDR=BUDRACEP G SETSECTH
 I +BUDETHN=2 S BUDR=BUDRACEP+8
SETSECTH ;
 S $P(BUDSECTH(1),U,BUDR)=$P($G(BUDSECTH(1)),U,BUDR)+1
 S $P(BUDSECTH(1),U,17)=$P($G(BUDSECTH(1)),U,17)+1
 S ^XTMP("BUD9RP7",BUDJ,BUDH,"PRGH",BUDAGEP,$P(^DPT(DFN,0),U),BUDCOM,DFN)=$P(BUDP,"*",2)_"#"_$P(BUDHIV,"*",2)
 Q
 ;
...
PRGRLST ;EP - list of pregnant females
 ...
 S BUDP=$$PREG(DFN,$$FMADD^XLFDT(BUDED,(-(30*20))),BUDED)
 I '$P(BUDP,U) Q  ;not pregnant
 S BUDHISPN=$$HISP^BUD9RPTC(DFN)
 S BUDHISP=$P($$HISP^BUD9RPTC(DFN),U,1)  ;1=hispanic 2=non hispanic
 ;S BUDHISP1=BUDHISP+2  ;set piece
 ;
 S BUDR1=$$RACE^BUD9RPTC(DFN)
 S BUDR=$P(BUDR1,U,1)  ;LINE
 S ^XTMP("BUD9RP7",BUDJ,BUDH,"PRGR",$P(BUDR1,U,5),BUDHISP,BUDCOM,BUDAGEP,$P(^DPT(DFN,0),U),DFN)=$P(BUDP,"*",2)
 Q
 ;
PRGELST ;EP - list of pregnant females
 ...
 S BUDP=$$PREG(DFN,$$FMADD^XLFDT(BUDED,(-(30*20))),BUDED)
 I '$P(BUDP,U) Q  ;not pregnant
 S BUDRACE=$$HISP^BUD9RPTC(DFN)
 I +BUDRACE=1 S BUDRACE="Hispanic or Latino"
 I +BUDRACE=2 S BUDRACE="All Others"
 S ^XTMP("BUD9RP7",BUDJ,BUDH,"PRGE",BUDRACE,BUDCOM,BUDAGEP,$P(^DPT(DFN,0),U),DFN)=$P(BUDP,"*",2)
 Q
 ;
