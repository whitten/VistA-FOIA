BUD0RP6B ; IHS/CMI/LAB - UDS REPORT DRIVER TABLE 6B 30 Dec 2010 8:09 PM ; 14 Dec 2010  1:24 PM
...
EN2 ;
 S BUDSITE=""
 S DIC="^BUDTSITE(",DIC(0)="AEMQ",DIC("A")="Enter your site: " D ^DIC
 I Y=-1 G PNC
 S BUDSITE=+Y
 I '$O(^BUDTSITE(BUDSITE,11,0)) W !!,"Warning:  There are no locations defined in the site parameter file for this",!,"site.  Report will not be accurate!" G EN
 S BUDTAXT="B6" D TAXCHK^BUD0XTCH
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BUD0RP6B",ZTDTH="",ZTDESC="UDS 10 REPORT TABLE 6B" D ^%ZTLOAD D EOJ Q
 Q
EOJ ;
 D EN^XBVK("BUD")
 Q
...
PROC ;EP - called from taskman
 S BUDJ=$J,BUDH=$H
 S ^XTMP("BUD0RP6B",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^BUD TABLE 6B LISTS"
 ;NOW LOOP THROUGH PATIENT FILE  for imms and paps
 S DFN=0 F  S DFN=$O(^AUPNPAT(DFN)) Q:DFN'=+DFN  D
 .;I DUZ=5634 Q:DFN'=118330
 .;I DUZ=5634 Q:$$HRN^AUPNPAT(DFN,DUZ(2))'=199926
 ...
 .D GETV^BUD0RPTD  ;get visits that meet criteria
...
PRGALST ;EP - list of pregnant females
 ...
 S X=$$AGB(BUDAGEP)
 S ^XTMP("BUD0RP6B",BUDJ,BUDH,"PRGA",X,BUDAGEP,$P(^DPT(DFN,0),U),BUDCOM,DFN)=$P(BUDP,"*",2)
 Q
AGB(N) ;
...
PRINT ;EP - called from taskman
 ...
 D HEADER^BUD0RPTP Q:BUDQUIT  D T6BH
 ...
 D LINE1
 I $Y>(IOSL-3) D HEADER^BUD0RPTP Q:BUDQUIT  D T6BH
 ...
 I $Y>(IOSL-12) D HEADER^BUD0RPTP Q:BUDQUIT  D T6BH
 ...
 W ?5,"December 31)",?23,"|",?45,"|",?65,"|",!
 D LINE
 I $Y>(IOSL-20) D HEADER^BUD0RPTP Q:BUDQUIT  D T6BH
 ...
 W ?5,"24-64 who had at",?23,"|",?45,"|",?65,"|",!
 W ?5,"least one Pap",?23,"|",?30,$$C($G(BUDSECTD("PTS"))),?45,"|",?50,$$C($G(BUDSECTD("PTS"))),?65,"|",?70,$$C($G(BUDSECTD("PAP"))),!
 W ?5,"test performed",?23,"|",?45,"|",?65,"|",!
 W ?5,"during the ",?23,"|",?45,"|",?65,"|",!
 W ?5,"measurement year",?23,"|",?45,"|",?65,"|",!
 W ?5,"or during one ",?23,"|",?45,"|",?65,"|",!
 W ?5,"of the two",?23,"|",?45,"|",?65,"|",!
 W ?5,"previous calendar",?23,"|",?45,"|",?65,"|",!
 W ?5,"years",?23,"|",?45,"|",?65,"|",!
 D LINE
 I $G(BUDPRGAL) S BUDGPG=0 D PRGAL^BUD0RP6I
 I $G(BUDIMM1L) S BUDGPG=0 D IMM1L^BUD0RP6I
 I $G(BUDIMM2L) S BUDGPG=0 D IMM2L^BUD0RP6I
 I $G(BUDPAP1L) S BUDGPG=0 D PAP1L^BUD0RP6P
 I $G(BUDPAP2L) S BUDGPG=0 D PAP2L^BUD0RP6P
 K ^XTMP("BUD0RP6B",BUDJ,BUDH)
 Q
T6BH ;
...
GENI ;general introductions
 ...
 W !,"December 31, 2010.  Based on the UDS defintion, to be considered a"
