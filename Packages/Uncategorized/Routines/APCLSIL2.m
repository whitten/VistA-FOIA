...
SETREC ;EP - called from
 ...
 S $P(APCLREC,",",17)=$$JDATE($$DSCHDATE(APCLV))
 ...
 ;S APCLADV="" I APCLHVAC S $P(APCLREC,",",23)=$$ADV(APCLV)
 S APCLASDM=$$ASTDM(DFN,$$VD^APCLV(APCLV))
 S $P(APCLREC,",",33)=$P(APCLASDM,U,1)
 S $P(APCLREC,",",34)=$P(APCLASDM,U,2)
 S APCLBMI=$$BMI(DFN,$$VD^APCLV(APCLV))
 S $P(APCLREC,",",35)=$$OB(DFN,$P(APCLBMI,U,1),$$AGE^AUPNPAT(DFN,$P(APCLBMI,U,6)))
 S $P(APCLREC,",",36)=$$PN^APCLSIL1(DFN,APCLV)
 S $P(APCLREC,",",37)=$$R($P(APCLBMI,U,1))
 S $P(APCLREC,",",38)=$$JDATE($P(APCLBMI,U,6))
 S %=$$PNEU(DFN,DT)
 S $P(APCLREC,",",39)=$P(%,U,2)
 S $P(APCLREC,",",40)=$P(%,U,1)
 S $P(APCLREC,",",41)=$$CLINIC^APCLV(APCLV,"C")
 S $P(APCLREC,",",43)=$P(APCLH1N1,U,2)
 ;S $P(APCLREC,",",44)=$P(APCLADVE,U,2)
 S $P(APCLREC,",",45)=$P(APCLSRD,U,2)
 S $P(APCLREC,",",46)=$P(APCLSRD,U,3)
 S $P(APCLREC,",",47)=$P(APCLSRD,U,4)
 S $P(APCLREC,",",48)=$P(APCLSRD,U,5)
 ;S $P(APCLREC,",",49)=$P(APCLAV9,U,2)
 ;S $P(APCLREC,",",50)=$P(APCLAV9,U,3)
 ;S $P(APCLREC,",",51)=$P(APCLAV9,U,4)
 ;S $P(APCLREC,",",52)=$P(APCLAV9,U,5)
 ;I $P(APCLHVAC,U,2)=125 S $P(APCLREC,",",53)=$$H1N1LIVE(APCLV)
...
PNEU(P,EDATE) ;EP
 I $G(P)="" Q ""
 NEW V,X,Y,F,I
 S V=$$LASTPNEU^APCLAPI4(P,,EDATE,"A")
 I V="" Q ""  ;no pneumovax ever
 S F=$P(V,U,5)
 S I=$P(V,U,6)
 I F="" Q $$JDATE($P(V,U))
 I I="" Q $$JDATE($P(V,U))
 S X=""
 I F=9000010.11 D  Q $$JDATE($P(V,U,1))_U_X
 .S I=$P(^AUPNVIMM(I,0),U,1)
 .S X=$P($G(^AUTTIMM(I,0)),U,3)
 Q $$JDATE($P(V,U,1))_U_$$VAL^XBDIQ1(F,I,.01)
 ;
OB(P,BMI,A) ;EP obese
 NEW S,R
 I $G(BMI)="" Q ""
...
BMI(P,EDATE) ;EP - get last calulable bmi as of EDATE and date of wt
 ...
 S AGE=$$AGE^AUPNPAT(P,EDATE) ;get age on edate
 S VALUE=""
...
WT(P,BDATE,EDATE) ;EP
 ...
 ...Q:$$ICD^ATXCHK(APCLICD,$O(^ATXAX("B","BGP GPRA PREGNANCY DIAGNOSES",0)),9)
...
ASTDM(P,EDATE) ;EP
 ...
 .Q:'$D(^AUPNPROB(X,0))
 .Q:$P(^AUPNPROB(X,0),U,12)'="A"  ;not on active list
...
DM(P,EDATE) ;EP
 ...
 I Q>1 Q "Y"
 Q ""
ADV(V) ;get all dx of TIER 1 in the 60 days after visit date.  take first 5
 NEW C,X,APCL,D,B,E,R,P1,P2,APCLD
 S C=0  ;counter
 S R=""  ;return value, comma delimited
 S D=$P($P(^AUPNVSIT(V,0),U),".")
 Q:D=""
 I $P(^AUPNVSIT(V,0),U,7)="H" S D=$$DSCHDATE^APCLV(V) I D="" S D=$$VD^APCLV(V)
 S B=D,E=$$FMADD^XLFDT(B,130)
 S P=$P(^AUPNVSIT(V,0),U,5)
 K APCL
 S X=P_"^ALL DX [SURVEILLANCE H1N1 ADV EV DXS;DURING "_$$FMTE^XLFDT(B)_"-"_$$FMTE^XLFDT(E) S E=$$START1^APCLDF(X,"APCL(")
 I '$D(APCL) Q ""
 ;RESET BY EARLIEST DATE
 S X=0,C=0 F  S X=$O(APCL(X)) Q:X'=+X!(C>4)  S APCLD($P(APCL(X),U),X)=APCL(X),C=C+1
 S C=0,P1=-1,P2=0
 S D=0 F  S D=$O(APCLD(D)) Q:D'=+D  D
 .S X=0 F  S X=$O(APCLD(D,X)) Q:X'=+X  D
 ..S C=C+1,P1=P1+2,P2=P2+2
 ..S B=$$JDATE($P(APCLD(D,X),U,1))
 ..S E=$P(APCLD(D,X),U,2)
 ..S $P(R,",",P1)=B
 ..S $P(R,",",P2)=E
 .Q
 Q R
H1N1LIVE(V) ;get all dx of H1N1 LIVE VIRUS DX within 40 days after visit date.  take first 3
 NEW C,X,APCL,D,B,E,R,P1,P2,APCLD
 S C=0  ;counter
 S R=""  ;return value, comma delimited
 S D=$P($P(^AUPNVSIT(V,0),U),".")
 Q:D=""
 I $P(^AUPNVSIT(V,0),U,7)="H" S D=$$DSCHDATE^APCLV(V) I D="" S D=$$VD^APCLV(V)
 S B=D,E=$$FMADD^XLFDT(B,40)
 S P=$P(^AUPNVSIT(V,0),U,5)
 K APCL
 S X=P_"^ALL DX [SURVEILL H1N1 ADV EV LV DXS;DURING "_$$FMTE^XLFDT(B)_"-"_$$FMTE^XLFDT(E) S E=$$START1^APCLDF(X,"APCL(")
 I '$D(APCL) Q ""
 ;RESET BY EARLIEST DATE
 S X=0,C=0 F  S X=$O(APCL(X)) Q:X'=+X!(C>2)  S APCLD($P(APCL(X),U),X)=APCL(X),C=C+1
 S C=0,P1=-1,P2=0
 S D=0 F  S D=$O(APCLD(D)) Q:D'=+D  D
 .S X=0 F  S X=$O(APCLD(D,X)) Q:X'=+X  D
 ..S C=C+1,P1=P1+2,P2=P2+2
