BUD8RP6P ; IHS/CMI/LAB - UDS REPORT DRIVER TABLE 6B ;
...
PAP1 ;EP
 ...
 D GENI
 D PAUSE
 W !!,"This report provides a list of all female patients ages 21-64 who have not"
...
PAP1L ;EP
 ...
 D PAP1H Q:BUDQUIT
 I '$D(^XTMP("BUD8RP6B",BUDJ,BUDH,"PAP1")) W !!,"No patients to report." Q
 D PAP1L1
...
PAP1L1 ;
 ...
 S BUDAGE="" F  S BUDAGE=$O(^XTMP("BUD8RP6B",BUDJ,BUDH,"PAP1",BUDAGE)) Q:BUDAGE=""!(BUDQUIT)  D
 .S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUD8RP6B",BUDJ,BUDH,"PAP1",BUDAGE,BUDNAME)) Q:BUDNAME=""!(BUDQUIT)  D
 ..S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUD8RP6B",BUDJ,BUDH,"PAP1",BUDAGE,BUDNAME,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ...S DFN=0 F  S DFN=$O(^XTMP("BUD8RP6B",BUDJ,BUDH,"PAP1",BUDAGE,BUDNAME,BUDCOM,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ....I $Y>(IOSL-3) D PAP1H Q:BUDQUIT
 ...
 ....S BUDTOT=BUDTOT+1
 ....S BUDALL=^XTMP("BUD8RP6B",BUDJ,BUDH,"PAP1",BUDAGE,BUDNAME,BUDCOM,DFN)
...
PAP1H1 ;
 ...
 W !,$$CTR("Patient List for Table 6B, Sections D, With Pap Test",80),!
 W $$CTR($P(^DIC(4,BUDSITE,0),U),80),!
 S X="Reporting Period: "_$$FMTE^XLFDT(BUDBD)_" to "_$$FMTE^XLFDT(BUDED) W $$CTR(X,80),!
 W $TR($J("",80)," ","-")
 I BUDP=0 D
 .W !,"List of all female patients age 21-64 who did not have a hysterectomy, had"
...
PAP2L ;EP
 S BUDP=0,BUDQUIT=0,BUDTOT=0
 D PAP2H Q:BUDQUIT
 I '$D(^XTMP("BUD8RP6B",BUDJ,BUDH,"PAP2")) W !!,"No patients to report." Q
 D PAP2L1
...
PAP2L1 ;
 ...
 S BUDAGE="" F  S BUDAGE=$O(^XTMP("BUD8RP6B",BUDJ,BUDH,"PAP2",BUDAGE)) Q:BUDAGE=""!(BUDQUIT)  D
 .S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUD8RP6B",BUDJ,BUDH,"PAP2",BUDAGE,BUDNAME)) Q:BUDNAME=""!(BUDQUIT)  D
 ..S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUD8RP6B",BUDJ,BUDH,"PAP2",BUDAGE,BUDNAME,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ...S DFN=0 F  S DFN=$O(^XTMP("BUD8RP6B",BUDJ,BUDH,"PAP2",BUDAGE,BUDNAME,BUDCOM,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ....I $Y>(IOSL-3) D PAP2H Q:BUDQUIT
 ....W !,$E($P(^DPT(DFN,0),U,1),1,25),?29,$S($$HRN^AUPNPAT(DFN,BUDSITE)]"":$$HRN^AUPNPAT(DFN,BUDSITE,2),1:$$HRN^AUPNPAT(DFN,DUZ(2),2)),?41,$E(BUDCOM,1,25),?68,BUDAGE,!
 ....S BUDTOT=BUDTOT+1
 ....S BUDALL=^XTMP("BUD8RP6B",BUDJ,BUDH,"PAP2",BUDAGE,BUDNAME,BUDCOM,DFN)
 ....W ?5,$P(BUDALL,U,1),?19,$P(BUDALL,U,2),?35,$P(BUDALL,U,3),?45,$P(BUDALL,U,4),?53,$E($P(BUDALL,U,5),1,11),?65,$E($P(BUDALL,U,6),1,15)
 Q
PAP2H ;
 G:'BUDGPG PAP2H1
...
PAP2H1 ;
 ...
 W !?3,$P(^VA(200,DUZ,0),U,2),?35,$$FMTE^XLFDT(DT),?70,"Page ",BUDGPG,!
 W !,$$CTR("***  RPMS Uniform Data System (UDS)  ***",80)
 W !,$$CTR("Patient List for Table 6B, Sections D, Without Pap Test",80),!
 W $$CTR($P(^DIC(4,BUDSITE,0),U),80),!
 S X="Reporting Period: "_$$FMTE^XLFDT(BUDBD)_" to "_$$FMTE^XLFDT(BUDED) W $$CTR(X,80),!
 W $TR($J("",80)," ","-")
 I BUDP=0 D
