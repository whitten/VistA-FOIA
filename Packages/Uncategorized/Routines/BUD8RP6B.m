BUD8RP6B ; IHS/CMI/LAB - UDS REPORT DRIVER TABLE 6B 30 Dec 2008 8:09 PM ;
...
EN2 ;
 ...
 S BUDTAXT="B6" D TAXCHK^BUD8XTCH
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BUD8RP6B",ZTDTH="",ZTDESC="UDS 08 REPORT TABLE 6B" D ^%ZTLOAD D EOJ Q
 Q
EOJ ;
 D EN^XBVK("BUD")
 Q
...
PROC ;EP - called from taskman
 S BUDJ=$J,BUDH=$H
 S ^XTMP("BUD8RP6B",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^BUD TABLE 6B LISTS"
...
PRGALST ; list of pregnant females
 ...
 S X=$$AGB(BUDAGEP)
 S ^XTMP("BUD8RP6B",BUDJ,BUDH,"PRGA",X,BUDAGEP,$P(^DPT(DFN,0),U),BUDCOM,DFN)=$P(BUDP,"*",2)
 Q
AGB(N) ;
...
PRINT ;EP - called from taskman
 ...
 D HEADER^BUD8RPTP Q:BUDQUIT  D T6BH
 ...
 D LINE1
 W ?2,"1",?5,"LESS THAN 15 YEARS",?45,"|",!
 D LINE1
 W ?2,"2",?5,"AGES 15-19",?45,"|",!
 D LINE1
 I $Y>(IOSL-3) D HEADER^BUD8RPTP Q:BUDQUIT  D T6BH
 W ?2,"3",?5,"AGES 20-24",?45,"|",! D LINE1
 I $Y>(IOSL-3) D HEADER^BUD8RPTP Q:BUDQUIT  D T6BH
 W ?2,"4",?5,"AGES 25-44",?45,"|",! D LINE1
 I $Y>(IOSL-3) D HEADER^BUD8RPTP Q:BUDQUIT  D T6BH
 W ?2,"5",?5,"AGES 45 AND OVER",?45,"|",! D LINE1
 I $Y>(IOSL-3) D HEADER^BUD8RPTP Q:BUDQUIT  D T6BH
 W ?2,"6",?5,"TOTAL PATIENTS (SUM LINES 1-5)",?45,"|",! D LINE1
 I $Y>(IOSL-12) D HEADER^BUD8RPTP Q:BUDQUIT  D T6BH
 ...
 W ?37,"|",?45,"(a)",?60,"|",?68,"(b)",!
 D LINE2
 W ?2,7,?5,"First Trimester",?37,"|",?60,"|",! D LINE2
 W ?2,8,?5,"Second Trimester",?37,"|",?60,"|",! D LINE2
 W ?2,9,?5,"Third Trimester",?37,"|",?60,"|",! D LINE2
 ;
 D HEADER^BUD8RPTP Q:BUDQUIT  D T6BH
 ;D LINE
 ...
 W ?5,"required vaccines",?23,"|",?45,"|",?65,"|",!
 W ?5,"who had their 2nd",?23,"|",?30,$$C($G(BUDSECTC("PTS"))),?45,"|",?50,$$C($G(BUDSECTC("PTS"))),?65,"|",?70,$$C($G(BUDSECTC("IMM"))),!
 W ?5,"birthday during",?23,"|",?45,"|",?65,"|",!
 W ?5,"measurement year",?23,"|",?45,"|",?65,"|",!
 W ?5,"(on or prior to",?23,"|",?45,"|",?65,"|",!
 W ?5,"31 December)",?23,"|",?45,"|",?65,"|",!
 D LINE
 I $Y>(IOSL-20) D HEADER^BUD8RPTP Q:BUDQUIT  D T6BH
 W $$CTR("SECTION D - PAP TESTS"),!
 D LINE
 W "PAP TEST",?23,"|",?26,"TOTAL NUMBER",?45,"|",?47,"NUMBER CHARTS",?65,"|",?67,"NUMBER OF",!
 W ?23,"|",?26,"OF FEMALE PATIENTS",?45,"|",?47,"SAMPLED OR EHR",?65,"|",?67,"PATIENTS",!
 W ?23,"|",?26,"21-64 YEARS OF AGE",?45,"|",?47,"TOTAL",?65,"|",?67,"TESTED",!
 W ?23,"|",?30,"(a)",?45,"|",?50,"(b)",?65,"|",?70,"(c)",!
 D LINE3
 W ?1,"10",?5,"Number of female",?23,"|",?45,"|",?65,"|",!
 W ?5,"patients aged",?23,"|",?45,"|",?65,"|",!
 W ?5,"21-64 who had at",?23,"|",?45,"|",?65,"|",!
 W ?5,"least on Pap test",?23,"|",?30,$$C($G(BUDSECTD("PTS"))),?45,"|",?50,$$C($G(BUDSECTD("PTS"))),?65,"|",?70,$$C($G(BUDSECTD("PAP"))),!
 W ?5,"performed during",?23,"|",?45,"|",?65,"|",!
 W ?5,"the measurement",?23,"|",?45,"|",?65,"|",!
 W ?5,"year or during one",?23,"|",?45,"|",?65,"|",!
 W ?5,"of the two ",?23,"|",?45,"|",?65,"|",!
 W ?5,"previous years",?23,"|",?45,"|",?65,"|",!
 D LINE
 I $G(BUDPRGAL) S BUDGPG=0 D PRGAL^BUD8RP6I
 I $G(BUDIMM1L) S BUDGPG=0 D IMM1L^BUD8RP6I
 I $G(BUDIMM2L) S BUDGPG=0 D IMM2L^BUD8RP6I
 I $G(BUDPAP1L) S BUDGPG=0 D PAP1L^BUD8RP6P
 I $G(BUDPAP2L) S BUDGPG=0 D PAP2L^BUD8RP6P
 ;K ^XTMP("BUD8RP6B",BUDJ,BUDH)
 Q
T6BH ;
