BUD0RPTS ; IHS/CMI/LAB - UDS REPORT DRIVER 12 Dec 2010 8:11 AM ;
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BUD0RPTS",ZTDTH="",ZTDESC="UDS 10 REPORT SEARCH TEMPLATE" D ^%ZTLOAD D EOJ Q
 Q
PRINTCP ;
 W !
 S X=$$CTR($P(^DIC(4,BUDSITE,0),U),60),$E(X,3)=$P(^VA(200,DUZ,0),U,2),$E(X,10)="UDS 2010",$E(X,70)="Cover Page" W !,X
 W !,"UDS No.  ",$P(^BUDTSITE(BUDSITE,0),U,2),?50,"Date Run: ",$$FMTE^XLFDT(DT)
...
NRY ;
 ...
 Q
STEMP ;EP
 S X=0 F  S X=$O(^XTMP("BUD0RPT1",BUDJ,BUDH,"3ATEMP",X)) Q:X'=+X  S ^DIBT(BUDSTMP,1,X)=""
 Q
