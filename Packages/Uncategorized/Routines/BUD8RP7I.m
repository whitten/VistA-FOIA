BUD8RP7I ; IHS/CMI/LAB - UDS REPORT DRIVER TABLE 6B ;
...
PRGH ;EP
 ...
 W !,"during the report period, and who have been diagnoses with HIV."
 W !
 Q
PRGHL ;EP
 S BUDP=0,BUDQUIT=0,BUDTOT=0
 D PRGHH Q:BUDQUIT
 I '$D(^XTMP("BUD8RP7",BUDJ,BUDH,"PRGH")) W !!,"No patients to report." Q
 D PRGHL1
...
PRGHL1 ;
 ...
 S BUDTOT=0
 S BUDA=0 F  S BUDA=$O(^XTMP("BUD8RP7",BUDJ,BUDH,"PRGH",BUDA)) Q:BUDA'=+BUDA!(BUDQUIT)  D
 .S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUD8RP7",BUDJ,BUDH,"PRGH",BUDA,BUDNAME)) Q:BUDNAME=""!(BUDQUIT)  D
 ..S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUD8RP7",BUDJ,BUDH,"PRGH",BUDA,BUDNAME,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ...S DFN=0 F  S DFN=$O(^XTMP("BUD8RP7",BUDJ,BUDH,"PRGH",BUDA,BUDNAME,BUDCOM,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ....I $Y>(IOSL-3) D PRGHH Q:BUDQUIT
 ....S BUDX=^XTMP("BUD8RP7",BUDJ,BUDH,"PRGH",BUDA,BUDNAME,BUDCOM,DFN)
 ...
 ....S BUDTOT=BUDTOT+1
 ....S BUDALL=^XTMP("BUD8RP7",BUDJ,BUDH,"PRGH",BUDA,BUDNAME,BUDCOM,DFN)
 ....S BUDPPV=$P(BUDALL,"#",1)
 ....S BUDHIV=$P(BUDALL,"#",2)
 ....S BUDHIV=$P(BUDHIV,"@",1)
...
PRGHH1 ;
 ...
 .W !
 W !,"PATIENT NAME",?25,"HRN",?41,"COMMUNITY",?53,"AGE",?59,"LAST HIV DX CD & DATE"
...
PRGR ;EP
 ...
 W !,$$CTR($$LOC,80)
 W !,$$CTR("UDS 2008",80)
 W !!,"All Pregnant Patients by Race (Table 7, Section A)",!
 D GENI
 D PAUSE
 W !!,"This report provides a list by race of patients that had pregnancy-"
...
PRGRL ;EP
 S BUDP=0,BUDQUIT=0,BUDTOT=0
 D PRGRH Q:BUDQUIT
 I '$D(^XTMP("BUD8RP7",BUDJ,BUDH,"PRGR")) W !!,"No patients to report." Q
 D PRGRL1
 I $Y>(IOSL-3) D PRGRH G:BUDQUIT PRGRLX
 W !!,"TOTAL PREGNANT PATIENTS BY RACE: ",BUDTOT,!
PRGRLX ;
 Q
RACE(R) ;EP
 I R="UNREP/REF" Q "Unreported"
 I R="ASIAN" Q "Asian"
...
PRGRL1 ;
 ...
 S BUDTOT=0
 S BUDRACE="" F  S BUDRACE=$O(^XTMP("BUD8RP7",BUDJ,BUDH,"PRGR",BUDRACE)) Q:BUDRACE=""!(BUDQUIT)  D PRGRL2
 Q
PRGRL2 ;
 S BUDSTOT=0
 W !,$$RACE(BUDRACE),!
 S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUD8RP7",BUDJ,BUDH,"PRGR",BUDRACE,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 .S BUDA="" F  S BUDA=$O(^XTMP("BUD8RP7",BUDJ,BUDH,"PRGR",BUDRACE,BUDCOM,BUDA)) Q:BUDA=""!(BUDQUIT)  D
 ..S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUD8RP7",BUDJ,BUDH,"PRGR",BUDRACE,BUDCOM,BUDA,BUDNAME)) Q:BUDNAME=""!(BUDQUIT)  D
 ...S DFN=0 F  S DFN=$O(^XTMP("BUD8RP7",BUDJ,BUDH,"PRGR",BUDRACE,BUDCOM,BUDA,BUDNAME,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ....I $Y>(IOSL-3) D PRGRH Q:BUDQUIT  W !,$$RACE(BUDRACE),!
 ....W !?2,$E($P(^DPT(DFN,0),U,1),1,20),?24,$S($$HRN^AUPNPAT(DFN,BUDSITE)]"":$$HRN^AUPNPAT(DFN,BUDSITE,2),1:$$HRN^AUPNPAT(DFN,DUZ(2),2)),?36,$E(BUDCOM,1,10),?47,BUDA,?51,$P($$RACE^BUD8RPTC(DFN),U,3)_"-"_$P($$RACE^BUD8RPTC(DFN),U,4),!
 ....S BUDTOT=BUDTOT+1,BUDSTOT=BUDSTOT+1
 ....S BUDALL=^XTMP("BUD8RP7",BUDJ,BUDH,"PRGR",BUDRACE,BUDCOM,BUDA,BUDNAME,DFN)
 ....S BUDPPV=$P(BUDALL,"#",1)
 ....F BUDX=1:1 S BUDV=$P(BUDPPV,U,BUDX) Q:BUDV=""!(BUDQUIT)  D
 .....I $Y>(IOSL-3) D PRGRH Q:BUDQUIT  W !,$$RACE(BUDRACE),!
 .....I $E(BUDV)="P" W ?5,BUDV,! Q
 .....S V=$P(BUDV,"|"),C=$P(BUDV,"|",2)
 .....W ?5,$$FMTE^XLFDT($P($P(^AUPNVSIT(V,0),U),".")),?19,C,?35,$$PRIMPROV^APCLV(V,"D"),?45,$P(^AUPNVSIT(V,0),U,7),?53,$$CLINIC^APCLV(V,"C"),?65,$E($$VAL^XBDIQ1(9000010,V,.06),1,15),!
 I $Y>(IOSL-4) D PRGRH Q:BUDQUIT  W !,$$RACE(BUDRACE),!
 W !,"Sub-Total ",$$RACE(BUDRACE),":  ",BUDSTOT,!
 Q
PRGRH ;
 G:'BUDGPG PRGRH1
 K DIR I $E(IOST)="C",IO=IO(0),'$D(ZTQUEUED) W ! S DIR(0)="EO" D ^DIR K DIR I Y=0!(Y="^")!($D(DTOUT)) S BUDQUIT=1 Q
PRGRH1 ;
 ...
 W !?3,$P(^VA(200,DUZ,0),U,2),?35,$$FMTE^XLFDT(DT),?70,"Page ",BUDGPG,!
 W !,$$CTR("***  RPMS Uniform Data System (UDS)  ***",80)
 W !,$$CTR("Patient List for Table 7, All Pregnant Patients by Race",80),!
 W $$CTR($P(^DIC(4,BUDSITE,0),U),80),!
 S X="Reporting Period: "_$$FMTE^XLFDT(BUDBD)_" to "_$$FMTE^XLFDT(BUDED) W $$CTR(X,80),!
 W $TR($J("",80)," ","-")
 I BUDP=0 D
 .W !,"List of all pregnant patients by race, with most recent pregnancy related"
 ...
 .W !
 W !?2,"PATIENT NAME",?24,"HRN",?36,"COMMUNITY",?47,"AGE",?51,"RACE*"
 W !?5,"VISIT DATE",?19,"DX OR SRV CD",?35,"PROV TYPE",?45,"SRV CAT",?53,"CLINIC",?65,"LOCATION"
 W !,$TR($J("",80)," ","-"),!
 S BUDP=1
 Q
...
PRGEL ;EP
 S BUDP=0,BUDQUIT=0,BUDTOT=0
 D PRGEH Q:BUDQUIT
 I '$D(^XTMP("BUD8RP7",BUDJ,BUDH,"PRGE")) W !!,"No patients to report." Q
 D PRGEL1
...
PRGEL1 ;
 ...
 S BUDTOT=0
 S BUDRACE="" F  S BUDRACE=$O(^XTMP("BUD8RP7",BUDJ,BUDH,"PRGE",BUDRACE)) Q:BUDRACE=""!(BUDQUIT)  D PRGEL2
 Q
PRGEL2 ;
 S BUDSTOT=0
 W !,BUDRACE,!
 S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUD8RP7",BUDJ,BUDH,"PRGE",BUDRACE,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 .S BUDA="" F  S BUDA=$O(^XTMP("BUD8RP7",BUDJ,BUDH,"PRGE",BUDRACE,BUDCOM,BUDA)) Q:BUDA=""!(BUDQUIT)  D
 ..S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUD8RP7",BUDJ,BUDH,"PRGE",BUDRACE,BUDCOM,BUDA,BUDNAME)) Q:BUDNAME=""!(BUDQUIT)  D
 ...S DFN=0 F  S DFN=$O(^XTMP("BUD8RP7",BUDJ,BUDH,"PRGE",BUDRACE,BUDCOM,BUDA,BUDNAME,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ....I $Y>(IOSL-3) D PRGEH Q:BUDQUIT  W !,BUDRACE,!
 ....W !?2,$E($P(^DPT(DFN,0),U,1),1,20),?24,$S($$HRN^AUPNPAT(DFN,BUDSITE)]"":$$HRN^AUPNPAT(DFN,BUDSITE,2),1:$$HRN^AUPNPAT(DFN,DUZ(2),2)),?36,$E(BUDCOM,1,10),?47,BUDA,?51,$P($$HISP^BUD8RPTC(DFN),U,2)_"-"_$P($$HISP^BUD8RPTC(DFN),U,3),!
 ....S BUDTOT=BUDTOT+1,BUDSTOT=BUDSTOT+1
 ....S BUDALL=^XTMP("BUD8RP7",BUDJ,BUDH,"PRGE",BUDRACE,BUDCOM,BUDA,BUDNAME,DFN)
 ....S BUDPPV=$P(BUDALL,"#",1)
 ....F BUDX=1:1 S BUDV=$P(BUDPPV,U,BUDX) Q:BUDV=""!(BUDQUIT)  D
 .....I $Y>(IOSL-3) D PRGEH Q:BUDQUIT  W !,BUDRACE,!
 .....I $E(BUDV)="P" W ?5,BUDV,! Q
