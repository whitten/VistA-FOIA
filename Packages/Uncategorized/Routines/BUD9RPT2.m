BUD9RPT2 ; IHS/CMI/LAB - UDS REPORT DRIVER 02 Dec 2007 11:27 AM 12 Dec 2009 8:12 AM ;
...
EN1 ;
 S BUDSITE=""
 W ! S DIC="^BUDNSITE(",DIC(0)="AEMQ",DIC("A")="Enter your site: " D ^DIC
 I Y=-1 D EOJ Q
 S BUDSITE=+Y
 ...
 I $D(DIRUT) D EOJ Q
 I Y D EOJ Q
 ;S XBRP="PRINT^BUD9RPTP",XBRC="PROC^BUD9RPTC",XBRX="EOJ^BUD9RPT2",XBNS="BUD"
 ;D ^XBDBQUE
 ;D EOJ
ZIS ;call to XBDBQUE
 K IOP,%ZIS
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BUD9RPT2",ZTDTH="",ZTDESC="UDS 09 REPORT" D ^%ZTLOAD D EOJ Q
 Q
M ;EP - called from option
 ...
 W !?5,"5   All patients by Service Category (Table 5 col B&C)"
 W !?5,"6   All Visits w/Uncategorized Primary Prov (Table 5 col B&C)"
 W !?5,"7   All Patients by Selected Primary Diagnosis (Table 6A)"
 W !?5,"8   ALL Patient Lists for LST1 -Sub Menu"
 S DIR(0)="L^1:8",DIR("A")="Include which Lists",DIR("B")=1 KILL DA D ^DIR KILL DIR
 I $D(DIRUT) D EOJ Q
 I Y["8" S (BUDT3A,BUDT3B,BUDT3BRL,BUDT4,BUDT5,BUDT6,BUDT3AL,BUDT5L,BUDT5L1,BUDT6L,BUDTZ,BUDTZL,BUDT5L2)=1
 I Y[2 S (BUDT3A,BUDT3AL)=1
 ;I Y[3 S (BUDT3B,BUDT3BEL)=1
 I Y[3 S (BUDT3B,BUDT3BRL)=1
 I Y[4 S (BUDT5,BUDT5L1)=1
 I Y[5 S (BUDT5,BUDT5L)=1
 I Y[6 S (BUDT5,BUDT5L2)=1
 I Y[7 S (BUDT6,BUDT6L)=1
 I Y=1!(Y["1,") S (BUDTZ,BUDTZL)=1
 ;I Y[7 S (BUDT5,BUDT5L3)=1
 G EN1
 Q
 ;
...
T3BRI ;intro for table 3B - RACE
 W !!,"ALL PATIENTS BY HISPANIC OR LATINO IDENTITY & RACE (TABLE 3B)"
 W !,"This report lists all patients who have at least on visit for the specified"
 W !,"time period that meet the above criteria.  Sorted by race/hispanic identity,"
 W !,"age, sex and community."
 W !,"Lists all visits that fit the definition.  Age is calculated as of June 30th"
 W !,"of the report year."
 Q
