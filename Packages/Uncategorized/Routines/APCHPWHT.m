APCHPWHT ; IHS/CMI/LAB -- create/modify health summary type ;
...
INIT ;EP -- init variables and list array
 K ^TMP($J,"APCHTYPE")
 S APCHC=0
 NEW X,Y,O,C,M,T,A,I,V,W,B
 S A="",T="",I="",B=""
 S X="STRUCTURE: " D S(X)
 S X="Order",$E(X,7)="Component" D S(X)
 S Y=0 F  S Y=$O(^APCHPWHT(APCHPWHT,1,Y)) Q:Y'=+Y  D
 .S A="",T="",B="",O=$P(^APCHPWHT(APCHPWHT,1,Y,0),U),C=$P(^APCHPWHT(APCHPWHT,1,Y,0),U,2),C=$P($G(^APCHPWHC(+C,0)),U,1)
 .I C="ALLERGIES" S A=Y
 .I C["TRANSPAR" S T=Y
...
AS ;EP
 D FULL^VALM1
 I '$$ALG(APCHPWHT) W !!,"You have not added Allergies as a component to this Patient Wellness Handout",!,"type.  Don't forgt to do so.",!
