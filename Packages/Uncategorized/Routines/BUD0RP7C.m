BUD0RP7C ; IHS/CMI/LAB - UDS REPORT PROCESSOR 01 Dec 2010 5:11 PM ;
...
DM ;EP - list of DM
 ...
 ;S BUDRACE=$$RACE^BUD0RP7I(BUDRACE)
 S BUDR=""
 ...
 I BUDRACEP=16!(+BUDETHN=3) S BUDR=17 G SETSECTC
...
SETSECTC ;
 ...
 S $P(BUDSECTC(9),U,18)=$P($G(BUDSECTC(9)),U,18)+1
 S $P(BUDSECTC(10),U,18)=$P($G(BUDSECTC(10)),U,18)+1
 ...
 I $G(BUDDMRL) S ^XTMP("BUD0RP7",BUDJ,BUDH,"DMR",BUDRACEP,+BUDETHN,BUDCOM,BUDAGE,$P(^DPT(DFN,0),U),DFN)=$P(BUDP,"^",2)
 ;I $G(BUDDMEL) S ^XTMP("BUD0RP7",BUDJ,BUDH,"DME",BUDETHNN,BUDCOM,BUDAGE,$P(^DPT(DFN,0),U),DFN)=$P(BUDP,"^",2)
 ;get last hgba1c value and set counters/lists
 S BUDHGB=$$HGBA1C(DFN,BUDBD,BUDED)
SET ;
 S BUDLINE=$P(BUDHGB,U)
 ;W !,DFN,"  ",BUDLINE
 S $P(BUDSECTC(BUDLINE),U,BUDR)=$P($G(BUDSECTC(BUDLINE)),U,BUDR)+1,$P(BUDSECTC(BUDLINE),U,18)=$P($G(BUDSECTC(BUDLINE)),U,18)+1
 S $P(BUDSECTF(BUDLINE),U,BUDETHN)=$P($G(BUDSECTF(BUDLINE)),U,BUDETHN)+1,$P(BUDSECTF(BUDLINE),U,9)=$P($G(BUDSECTF(BUDLINE)),U,9)+1
 I $G(BUDDMR1L),BUDLINE=11 S ^XTMP("BUD0RP7",BUDJ,BUDH,"DMR1",BUDRACEP,+BUDETHN,BUDCOM,BUDAGE,$P(^DPT(DFN,0),U),DFN)=$P(BUDP,"^",2)_"^"_$P(BUDHGB,U,2)
 I $G(BUDDMR2L),BUDLINE=12 S ^XTMP("BUD0RP7",BUDJ,BUDH,"DMR2",BUDRACEP,+BUDETHN,BUDCOM,BUDAGE,$P(^DPT(DFN,0),U),DFN)=$P(BUDP,"^",2)_"^"_$P(BUDHGB,U,2)
 I $G(BUDDMR3L),BUDLINE=13 S ^XTMP("BUD0RP7",BUDJ,BUDH,"DMR3",BUDRACEP,+BUDETHN,BUDCOM,BUDAGE,$P(^DPT(DFN,0),U),DFN)=$P(BUDP,"^",2)_"^"_$P(BUDHGB,U,2)
 ;I $G(BUDDME1L),BUDLINE=11 S ^XTMP("BUD0RP7",BUDJ,BUDH,"DME1",BUDETHNN,$P(^DPT(DFN,0),U),BUDCOM,DFN)=$P(BUDP,"^",2)_"^"_$P(BUDHGB,U,2)
 ;I $G(BUDDME2L),BUDLINE=12 S ^XTMP("BUD0RP7",BUDJ,BUDH,"DME2",BUDETHNN,$P(^DPT(DFN,0),U),BUDCOM,DFN)=$P(BUDP,"^",2)_"^"_$P(BUDHGB,U,2)
 ;I $G(BUDDME3L),BUDLINE=13 S ^XTMP("BUD0RP7",BUDJ,BUDH,"DME3",BUDETHNN,$P(^DPT(DFN,0),U),BUDCOM,DFN)=$P(BUDP,"^",2)_"^"_$P(BUDHGB,U,2)
 Q
 ;
HGBA1C(P,BDATE,EDATE) ;EP
 NEW BUDG,BUDT,BUDC,E,%,L,T,BUDLT,D,X,J,C,G
 S BUDC=0
 S G=$$CPT^BUD0DU(P,BDATE,EDATE,$O(^ATXAX("B","BUD HGBA1C CPTS",0)),5)
 ...
 S C=0,C=$O(BUDT(D,C))
 S X=$P(BUDT(D,C),U,1)
 I $$UP^XLFSTR(X)="COMMENT" Q 13_U_$P(BUDT(D,C),U,2)_" "_$$DATE^BUD0UTL1(9999999-D)_" (no result) "_X
 I X="" D  Q G
 .S G=""
 .I $P(BUDT(D,C),U,2)="CPT 3046F" S G=13_U_$P(BUDT(D,C),U,2)_" "_$$DATE^BUD0UTL1(9999999-D) Q
 .I $P(BUDT(D,C),U,2)="CPT 3045F" S G=12_U_$P(BUDT(D,C),U,2)_" "_$$DATE^BUD0UTL1(9999999-D) Q
 .I $P(BUDT(D,C),U,2)="CPT 3044F" S G=11_U_$P(BUDT(D,C),U,2)_" "_$$DATE^BUD0UTL1(9999999-D) Q
 .I $P(BUDT(D,C),U,2)="CPT 83036" S G=13_U_$P(BUDT(D,C),U,2)_" "_$$DATE^BUD0UTL1(9999999-D) Q
 .I $P(BUDT(D,C),U,2)="CPT 83037" S G=13_U_$P(BUDT(D,C),U,2)_" "_$$DATE^BUD0UTL1(9999999-D) Q
 .S G=13_U_$P(BUDT(D,C),U,2)_" DID NOT HAVE A RESULT "_$$DATE^BUD0UTL1(9999999-D) Q
 S X=$$STRIP^XLFSTR(X," ")  ;strip spaces
 I X[">9" Q 13_U_$P(BUDT(D,C),U,2)_" "_X_" "_X_$$DATE^BUD0UTL1(9999999-D)
 S X=$$STV(X)
 I X="" Q 13_U_$P(BUDT(D,C),U,2)_" DID NOT HAVE A RESULT "_$$DATE^BUD0UTL1(9999999-D)
 I +X>9 Q 13_U_$P(BUDT(D,C),U,2)_" "_X_" "_$$DATE^BUD0UTL1(9999999-D)
 I +X<7 Q 11_U_$P(BUDT(D,C),U,2)_" "_X_" "_$$DATE^BUD0UTL1(9999999-D)
 Q 12_U_$P(BUDT(D,C),U,2)_" "_X_" "_$$DATE^BUD0UTL1(9999999-D)
 ;Q 0_U_$P(BUDT(D,C),U,2)_" "_$$DATE^BUD0UTL1(9999999-D)_" result: "_X
 ;
RACE(R) ;EP
 I R="Unreported" Q 8
 I R="Asian" Q 1
...
PROBDMM ;now check for med
 K BUDMEDS1 D GETMEDS^BUD0UTL2(P,BDATE,EDATE,"BUD DIABETES MEDS TAX","BUD DIABETES MEDS NDC",,,.BUDMEDS1)
