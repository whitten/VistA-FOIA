BUD9RP6Y ; IHS/CMI/LAB - UDS REPORT PROCESSOR 01 Dec 2009 4:03 PM ;
...
HEPCONT ;check allergy tracking
 ...
 .Q:'$$ANAREACT^BUD9RP6C(X)  ;quit if anaphylactic is not a reaction/sign/symptom
 .I N["BAKER'S YEAST"!(N["BAKERS YEAST") S G="HEP B Contraindiction: "_$$DATE^BUD9UTL1($P($P($G(^GMR(120.8,X,0)),U,4),"."))_"  Allergy Tracking: "_N
 I G]"" Q G
 ;now check immunization package
 F BUDZ=8,44,45,51,104,110 S X=$$ANCONT^BUD9RP6C(P,BUDZ,EDATE) Q:X]""
 I X]"" Q "HEP B Contraindication IM package: "_$$DATE^BUD9UTL1($P(X,U))_" "_$P(X,U,2)
 ;now check for evidence of disease
HEPEVID ;
 K BUDG S %=P_"^LAST DX [BGP HEP EVIDENCE;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BUDG(")
 I $D(BUDG(1)) Q "HEP B Evidence: "_$P(BUDG(1),U,2)_" on "_$$DATE^BUD9UTL1($P(BUDG(1),U))
 S X=$$PLTAX^BUD9DU(P,"BGP HEP EVIDENCE") I X Q "HEP B Evidence: Problem List "_$P(X,U,2)
...
HEPIMM ;get all immunizations
 ...
 ....I Y=90723!(Y=90740)!(Y=90744)!(Y=90747)!(Y=90748) S BUDOPV(9999999-$P(ED,"."))="CPT: "_Y_" on "_$$DATE^BUD9UTL1((9999999-$P(ED,"."))),BUDAPOV(9999999-$P(ED,"."))="CPT: "_Y_" on "_$$DATE^BUD9UTL1((9999999-$P(ED,".")))
...
HIB(P,BDATE,EDATE) ;EP
 ...
 NEW X,G,N,BUDG,BUDX,BUDC,BUDOPV,BUDAPOV,C,BD,ED,V,Y,E
 ;now check for evidence of disease
 F BUDZ=22,46,47,48,49,50,51,120 S X=$$ANCONT^BUD9RP6C(P,BUDZ,EDATE) Q:X]""
 I X]"" Q "HIB Contraindication IM package: "_$$DATE^BUD9UTL1($P(X,U))_" "_$P(X,U,2)
HIBEVID ;
 K BUDG S %=P_"^LAST DX [BGP HIB EVIDENCE;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BUDG(")
 I $D(BUDG(1)) Q "HIB Evidence: "_$P(BUDG(1),U,2)_" on "_$$DATE^BUD9UTL1($P(BUDG(1),U))
 S X=$$PLTAX^BUD9DU(P,"BGP HIB EVIDENCE") I X Q "HIB Evidence: Problem List "_$P(X,U,2)
 ;now get imms and see if there are 3
 K BUDC,BUDG,BUDX
 K BUDOPV,BUDAPOV
HIBIMM ;get all immunizations
 ...
 ....S BUDOPV(9999999-$P(ED,"."))="CPT: "_Y_" on "_$$DATE^BUD9UTL1((9999999-$P(ED,"."))),BUDAPOV(9999999-$P(ED,"."))="CPT: "_Y_" on "_$$DATE^BUD9UTL1((9999999-$P(ED,".")))
 ..S X=0 F  S X=$O(^AUPNVTC("AD",V,X)) Q:X'=+X  D
...
VARC ;
 ...
 I $D(BUDG(1)) Q "Varicella CONTRA DX: "_$P(BUDG(1),U,2)_" on "_$$DATE^BUD9UTL1($P(BUDG(1),U))
 S X=$$PLTAX^BUD9DU(P,"BGP MMR CONTRAINDICATIONS") I X Q "VAR CONTRA DX: "_$P(X,U,2)_" on Problem List"
 S G=""
 S X=0 F  S X=$O(^GMR(120.8,"B",P,X)) Q:X'=+X!(G)  D
 .Q:$P($P($G(^GMR(120.8,X,0)),U,4),".")>EDATE  ;entered after 2ND birthday
 .S N=$P($G(^GMR(120.8,X,0)),U,2),N=$$UP^XLFSTR(N)
 .Q:'$$ANAREACT^BUD9RP6C(X)  ;quit if anaphylactic is not a reaction/sign/symptom
 .I N["NEOMYCIN" S G="VAR Contraindiction: "_$$DATE^BUD9UTL1($P($P($G(^GMR(120.8,X,0)),U,4),"."))_"  Allergy Tracking: "_N
 I G]"" Q G
 F BUDZ=21,94 S X=$$MMRCONT^BUD9RP6C(P,BUDZ,EDATE) Q:X]""
 I X]"" Q "Varicella Contraindication: "_$P(X,U,2)_" on "_$$DATE^BUD9UTL1($P(X,U,1))_" Immunization Package"
VAREVID ;
 ;any evidence of VAR?
 K BUDG S %=P_"^LAST DX [BGP VARICELLA EVIDENCE;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BUDG(")
 I $D(BUDG(1)) Q "Varicella Evidence: "_$P(BUDG(1),U,2)_" on "_$$DATE^BUD9UTL1($P(BUDG(1),U))
 S X=$$PLTAX^BUD9DU(P,"BGP VARICELLA EVIDENCE") I X Q "Varicella Evidence: Problem List "_$P(X,U,2)
VARI ;
 K BUDC,BUDG,BUDX
 ...
 S C="21^94"
 K BUDX D GETIMMS^BUD9RP6C(P,BDATE,EDATE,C,.BUDX)  ;before 2nd birthday
 ...
 ..Q:'$D(^AUPNVCPT("AD",V))
 ..S X=0 F  S X=$O(^AUPNVCPT("AD",V,X)) Q:X'=+X  D
 ...S Y=$P(^AUPNVCPT(X,0),U),Y=$P($$CPT^ICPTCOD(Y),U,2) I Y="90710"!(Y="90716") S BUDVAR="Varicella CPT: "_Y_" on "_$$DATE^BUD9UTL1($P($P(^AUPNVSIT(V,0),U),"."))
 I BUDVAR]"" Q "Varicella "_BUDVAR
 ;
 ;K BUDG S %=P_"^ALL DX V05.4;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BUDG(")
 ;I $D(BUDG(1)) Q "Varicella DX: "_$P(BUDG(1),U,2)_" on "_$$DATE^BUD9UTL1($P(BUDG(1),U))
 Q ""
 ;
PNEU(P,BDATE,EDATE) ;EP
 ...
 K BUDC,BUDG,BUDX
 F BUDZ=33,100,109 S X=$$ANCONT^BUD9RP6C(P,BUDZ,EDATE) Q:X]""
 I X]"" Q "Pneumovax Contraindication IM package: "_$$DATE^BUD9UTL1($P(X,U))_" "_$P(X,U,2)
 K BUDOPV,BUDAPOV
PNEUIMM ;get all immunizations
 ...
 ..Q:'$D(^AUPNVSIT(V,0))
 ..S X=0 F  S X=$O(^AUPNVCPT("AD",V,X)) Q:X'=+X  D
 ...S Y=$P(^AUPNVCPT(X,0),U),Y=$P($$CPT^ICPTCOD(Y),U,2) I Y=90669 D
 ....S BUDOPV(9999999-$P(ED,"."))="CPT: "_Y_" on "_$$DATE^BUD9UTL1((9999999-$P(ED,"."))),BUDAPOV(9999999-$P(ED,"."))="CPT: "_Y_" on "_$$DATE^BUD9UTL1((9999999-$P(ED,".")))
 ..S X=0 F  S X=$O(^AUPNVTC("AD",V,X)) Q:X'=+X  D
 ...S Y=$P(^AUPNVTC(X,0),U,7) Q:'Y  S Y=$P($$CPT^ICPTCOD(Y),U,2) I Y=90669 D
 ....S BUDOPV(9999999-$P(ED,"."))="CPT: "_Y_" on "_$$DATE^BUD9UTL1((9999999-$P(ED,"."))),BUDAPOV(9999999-$P(ED,"."))="CPT: "_Y_" on "_$$DATE^BUD9UTL1((9999999-$P(ED,".")))
 ;now check to see if they are all spaced 10 days apart, if not, kill off the odd ones
