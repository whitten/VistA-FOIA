...
RECENTBP ;EP
 ...
 I '$D(APCHWT) D S^APCHPWH1("No blood pressure values have been recorded.  We recommend that you have"),S^APCHPWH1("your blood pressure rechecked at your next visit.") Q
...
PROBLIST ;EP
 ...
 .S APCHY=$$ICDDX^ICDCODE(APCHY)
 .S APCHT=$P(APCHY,U,4)
 .S APCHI=$P(APCHY,U,2)
 .S APCHD=$$VAL^XBDIQ1(9000011,APCHN,.13) I APCHD]"" S APCHD="Onset: "_APCHD
 .K ^UTILITY($J,"W") S X=APCHT,DIWL=0,DIWR=48 D ^DIWP
 .S X=" "_APCHI,$E(X,10)=$G(^UTILITY($J,"W",0,1,0)),$E(X,60)=APCHD D S^APCHPWH1(X)
 .F F=2:1:$G(^UTILITY($J,"W",0)) S X="",$E(X,10)=^UTILITY($J,"W",0,F,0) D S^APCHPWH1(X)
...
FAMHX ;EP
 ...
 .S APCHT=$P(APCHY,U,4)
 .S APCHI=$P(APCHY,U,2)
 .S APCHD=$$VAL^XBDIQ1(9000014,APCHN,.11)
 .S APCHR=$$VALI^XBDIQ1(9000014,APCHN,.09)
 .I 'APCHR S APCHRD="RELATION UNKNOWN" G FAMHX1
...
FAMHX1 .K ^UTILITY($J,"W") S X=APCHT,DIWL=0,DIWR=30 D ^DIWP
 .S X=APCHRD,$E(X,23)=APCHS,$E(X,33)=$G(^UTILITY($J,"W",0,1,0)),$E(X,66)=APCHD D S^APCHPWH1(X)
...
GETFHX ;
 ...
 Q
APPTS ;EP
 NEW APCHAPPT,X,Y,APCHCLN,APCHSDAT,APCHSVDT,APCHX,APCHSAM,APCHSVT,APCHSN,F
...
APPTS1 ;
 ...
 I F S F=$S($P($G(^APCCCTRL(DUZ(2),0)),U,13):$P(^APCCCTRL(DUZ(2),0),U,13),1:$P(^DIC(4,DUZ(2),0),U,1))
 S X=$$FMTE^XLFDT($P(APCHSVDT,".")),$E(X,13)=APCHSVT,$E(X,23)=APCHSCN I F]"" S X=X_"  ("_F_")"
 D S^APCHPWH1(X)
 Q
PEDSCRN ;EP
...
DENTAL ;
 ...
 I APCHLDE="" D
 .D S^APCHPWH1("We recommend that your child have an dental checkup.  Be sure to discuss")
 .D S^APCHPWH1("this with your child's provider.")
 I APCHLDE]"" D
 .D S^APCHPWH1("Your child's last dental checkup was performed on "_$$FMTE^XLFDT($P(APCHLDE,U))_".")
 Q
DMPV(P) ;how many dm povs?
 NEW X,E,APCHX
...
ANTICOAG ;EP
 ...
 ;reorder by date
 K APCHD
 S G=0 F  S G=$O(APCHV(G)) Q:G'=+G  S APCHD(9999999-$P(APCHV(G),U,1),$P(APCHV,U,4))=APCHV(G)
 D S^APCHPWH1("Your last 3 INR results were:")
 I '$D(APCHD) D  I 1
 .D S^APCHPWH1("  None Documented.  We recommend that you ask your ",0,0,31),S^APCHPWH1("health care provider about your INR results.",0,0,31)
 E  D
 .S G=0,C=0 F  S G=$O(APCHD(G)) Q:G'=+G  D
 ..S X=0 F  S X=$O(APCHD(G,X)) Q:X'=+X  D
 ...S C=C+1 D S^APCHPWH1($P(APCHD(G,X),U,3)_" on "_$$FMTE^XLFDT($P(APCHD(G,X),U,1)),$S(C>1:1,1:0),0,31)
 K APCHMEDS
